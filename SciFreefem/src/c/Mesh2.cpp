...
#include "clerror.h"  // for gestion of exceptions
...
{ 
  char terr [15];
  GestChar t("Fatal error in the meshgenerator ");
  sprintf(terr,"%i",Err);
  t = t + "(" + terr + ")";
  throw ErrorExec(t.Data());
}
...
{ // in: B 
  ...
  while(t->det  > 0 ) 
    { 
      assert( kkkk++ < 2000 /* infinity loop */ ); 
      j= OppositeVertex[jj];
...
{ //  SplitEdge
  ...
  for (;;) {
    ...
    if (iedge < 0) {
      ...
      dt[VerticesOfTriangularEdge[iedge][1]] = deti;// we take the traingle by the other side
...
#endif
  if (ToSwap) NbSwap++,
		::swap(t1,a1,t2,a2,&s1,&s2,ndet1,ndet2);
...
#endif 
  if( OnSwap ) 
    ::swap(t1,a1,t2,a2,s1,s2,det1,det2);
  else {
    NbUnSwap ++;
    t1->SetMarkUnSwap(a1);     
  }
...
Real8  Vertex::Smoothing(Triangles & Th,const Triangles & BTh,Triangle  * & tstart ,Real8 omega)
{
  ...
  while (ok) 
	...
	tria->det =  ::det( (*tria)[0],(*tria)[1]  ,(*tria)[2]);
	if (loop) 
	  {
	    Vertex *v0,*v1,*v2,*v3;
	    if (tria->det<0) ok =1;			       
	    ...
	    else if ( (double)tria->det < tria->det/2 ) ok=1;
...
void Triangles::Add( Vertex & s,Triangle * t, Icoor2 * det3) 
{
  ...
  Triangle * tt[3]; // the 3 new traingles
  ...
  if (!det3) { 
    ...
    if ( infv<0 ) {
      ...
      det3[0]=  &s0 ? -1  : ::det(s ,s1,s2) ;
      det3[1]=  &s1 ? -1 : ::det(s0,s ,s2) ;
      det3[2]=  &s2 ? -1 : ::det(s0,s1,s ) ;}}
...
{ // Triangles::NewPoints`
  Real8 seuil= 1.414/2 ;// for two neart point 
  if (verbosity>2) 
  ...
  Int4 NbTSwap =0;
  Triangle *t;
  // generation of the list of next traingle 
  ...
  do {
	...
	{// for all the new point
	  ...
	  Real4 hx,hy;
	  vi.m.Box(hx,hy);
	  Icoor1 hi=(Icoor1) (hx*coefIcoor)
	    ,hj=(Icoor1) (hy*coefIcoor);
	  if (!quadtree->ToClose(vi,seuil,hi,hj)) {
	    // a good new point 
...
{ // Triangles::NewPointsOld
  ...
  do {
	...
	for(j=0;j<3;j++)
	  { // for each edge 
	    ...
	    for (Int4 iv=NbvOld;iv<nbvNew;iv++) {
	      ...
	      vertices[nbv].ref=nbv;// circular link
	      R2 C =  vertices[iv].r;
	      vertices[nbv].r =  C;
		...
		{ // the two parameter is i0 and i1 
		  ...
		  if ( d01 < seuil) 
		    if (i1<nbvold) {
		      ...
		      for  (ip=i0;i0 != (ipp = vertices[ip].ref);ip=ipp)
			vertices[ip].ref = -1;// mark remove
		      vertices[ip].ref = -1;// mark remove
		    }
		    else {
		      ...
		      for  (ip0=i0;i0 != (ipp0 = vertices[ip0].ref);ip0=ipp0) kk0++;
		      for  (ip1=i1;i1 != (ipp1 = vertices[ip1].ref);ip1=ipp1) kk1++;
		
		      register Real8 w0 = ((Real8) kk0)/(kk0+kk1);
		      register Real8 w1 = ((Real8) kk1)/(kk0+kk1);

		      // make a circular link
		      Exchange(vertices[i0].ref,vertices[i1].ref);
		      // the new coordinate 
		      R2 C //= vertices[i0] ;
			...
			cout << "\n ref = "<< vertices[i0].ref << " " <<vertices[i1].ref <<endl;
		      ...
		      for  (ip0=i0;i0 != (ipp0 = vertices[ip0].ref);ip0=ipp0)
			vertices[ip0].r = C;	      
		      vertices[ip0].r = C;
		    }
		}
    ...
    for (i=nbvold;i<nbv;i++) 
      if (vertices[i].ref>=0) {// good points
	//  cout <<" i = " << i ;
	for  (ip=i;i != (ipp = vertices[ip].ref);ip=ipp)
	  vertices[ip].ref = -1;// mark remove
	vertices[ip].ref = -1;// mark remove
	// cout << i << " ---> " << kkk << endl;        
...
void Triangles::ForceBoundary()
{
  ...
  if (k!=0) {
    cerr << " there is  " << k << " 0 triangles " << endl;
    MeshError(11);}
  ...
  for (Int4 i = 0; i < nbe; i++) 
    {
      ...
      if (nbswp) nbfe++;
      //	cout << " Edge " << i << " v0 =  " << Number(edges[i][0]) 
      //	     <<" v1= " << Number(edges[i][1]) << endl;
    }
		  
  
  if (k!=0) {
    cerr << " there is " << k << " lost edges " << endl;
...
void Triangles::FindSubDomain(int OutSide=0)
{
  ...
  for ( it=0;it<nbt;it++)  { 
	...
	{ while ( HeapArete[i]--) // boucle sur les 3 aretes 
		...
		{
		  Triangle * ta = tc->TriangleAdj(na) ; // nÊ triangle adjacent
...
void Triangles::ReNumberingTheTriangleBySubDomain()
{
  ...
  // move the traingles  without a copy of the array 
  // be carefull not trivial code 
...
void Triangles::GeomToTriangles1(Int4 inbvx) 
{ 
...
Triangles::~Triangles() 
{
  ...
  if(edgescomponante) delete [] edgescomponante;
  ...
  //  if (OutSidesTriangles) delete OutSidesTriangles;
  if (VertexOnBThVertex) delete [] VertexOnBThVertex;
  if (VertexOnBThEdge) delete [] VertexOnBThEdge;
  
  // if (allocGeometry) delete  allocGeometry;
  // if (&Gh) delete &Gh;  PB because we remove the Gh record 
  if (&Gh) 
    {
...
void Triangles::FillHoleInMesh() 
{
  ...
  // generation of the integer coor
  {
    ...
    // copy of the outside triangles in savetraingles 
    for (i=0;i<nbt;i++)
...
Triangles::Triangles(double * tNode, int nbNode, int * tTriangle, int nbTriangle)
  : Gh(*(new Geometry())), BTh(*this)
{
  ...
  for (i = 0; i < nbv; i++)
    {
      vertices[i].r.x = tNode[i];
      vertices[i].r.y = tNode[i + nbv];
      vertices[i].ref = (Int4) tNode[i + 2 * nbv];
    }
    
  //YC Int4 i1 [nbt],i2 [nbt],i3 [nbt];
  for (i = 0; i < nbt; i++)
    {
      //YC i1[i] = tTriangle[i];
      //YC i2[i] = tTriangle[i + nbt];
      //YC i3[i] = tTriangle[i + 2 * nbt];
      //YC triangles[i].color = tTriangle[i + 3 * nbt];

      triangles[i] = Triangle(this, tTriangle[i] - 1, tTriangle[i + nbt] - 1, tTriangle[i + 2 * nbt] - 1);
      triangles[i].color = tTriangle[i + 3 * nbt];
    }

  //YC for (i = 0; i < nbt; i++)
  //YC    triangles[i] = Triangle(this,i1[i] - 1,i2[i] - 1,i3[i] - 1);
