...
public:
  ...
  void  Abs(){lambda1=::Abs(lambda1),lambda2=::Abs(lambda2);}
  void  Min(double a) { lambda1=::Min(a,lambda1); lambda2=::Min(a,lambda2) ;}
  void  Max(double a) { lambda1=::Max(a,lambda1); lambda2=::Max(a,lambda2) ;}
  ...
  Real8 hmin() const {return sqrt(1/::Max3(lambda1,lambda2,1e-30));}
  Real8 hmax() const {return sqrt(1/::Max(::Min(lambda1,lambda2),1e-30));}
  Real8 lmax() const {return::Max3(lambda1,lambda2,1e-30);}
  Real8 lmin() const {return ::Max(::Min(lambda1,lambda2),1e-30);}
...
inline void  MatVVP2x2::BoundAniso2(const Real8 coef) 
{
  if (coef<=1.00000000001) 
    if (lambda1 < lambda2)
      lambda1 = ::Max(lambda1,lambda2*coef);
    else
      lambda2 = ::Max(lambda2,lambda1*coef);
  else  // a verifier 
    if (lambda1 > lambda2)
      lambda1 = ::Min(lambda1,lambda2*coef);
    else
      lambda2 = ::Min(lambda2,lambda1*coef);
}
