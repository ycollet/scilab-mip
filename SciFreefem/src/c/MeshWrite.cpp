...
void Triangles::Write(char * filename,const TypeFileMesh typein )
{
  ...
  if (type==AutoMesh)
    {
      ...
      if      (!strcmp(filename + lll - 7,".am_fmt")) type = am_fmtMesh;
      else if (!strcmp(filename + lll - 6,".amdba"))  type = amdbaMesh;
      else if (!strcmp(filename + lll - 3,".am"))     type = amMesh;
      else if (!strcmp(filename + lll - 5,".nopo"))   type = NOPOMesh;
      else if (!strcmp(filename + lll - 4,".msh"))    type = mshMesh;
      else if (!strcmp(filename + lll - 4,".ftq"))    type = ftqMesh;
      else if (!strcmp(filename + lll - 7,".AM_FMT")) type = am_fmtMesh;
      else if (!strcmp(filename + lll - 6,".AMDBA"))  type = amdbaMesh;
      else if (!strcmp(filename + lll - 3,".AM"))     type = amMesh;
      else if (!strcmp(filename + lll - 5,".NOPO"))   type = NOPOMesh;
      else if (!strcmp(filename + lll - 4,".MSH"))    type = mshMesh;
  ...
  if (verbosity>1)
	...
	default: 
	  cerr << endl 
	       <<  " Unkown type mesh file " << type << " for Writing " << filename <<endl;
	  MeshError(1);
	}     
  ...
  if (f)
    switch (type) 
      ...
      default: 
	cerr << " Unkown type mesh file " << type << " for Writing " << filename <<endl;
	MeshError(1);
      }
  else
    {
