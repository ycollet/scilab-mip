...
void  Triangles::Read_ftq(MeshIstream & f_in)
{
  ...
  for ( i=0;i<nbv;i++)
    {
      f_in >> vertices[i].r.x >>   vertices[i].r.y >> vertices[i].ref;
    }
}
  ...
  for ( i=0;i<nbv;i++)
    {
      ...
      assert( j >0 && j <= nbv);
      j--;
      f_in >> vertices[j].r.x >>   vertices[j].r.y >> vertices[j].ref;
      vertices[j].m=M1;
    }
...
Triangles::Triangles(char * filename,Real8 cutoffradian) 
