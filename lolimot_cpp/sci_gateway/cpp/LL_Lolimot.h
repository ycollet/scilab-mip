...
typedef float (*PtrToFctResidu)(vector<float> &, vector<float> &, vector<float> &);
...
class LL_Lolimot
{
 public:
  LL_Lolimot();
  LL_Lolimot(string name, int nbMaxPartitions, float residuGapPercentage);
  virtual ~LL_Lolimot();
  ...
  vector<LL_Dimension*> getDimensionSet() const;
  LL_Dimension *        getDimension(int i) const;
  LL_Dimension *        addDimension(string name, float min, float max, float ecartType, int nbDiscretisations);
  int                   cleanDimensions();
  
  // managing the measurements
  vector<LL_Mesure*>    getMesureSet() const;
  ...
  vector<LL_Partition*> getPartitionSet() const;
  LL_Partition *        getPartition(int i) const;
  LL_Partition *        addPartition();
  string                getRandName(string Prefix, unsigned int NbRandChar);
  int                   cleanPartitions();

  // managing the weights
  vector<float>         getWeightSet() const;
  ...
  bool                  exportAllPartitions(string path, string Filename);
  ...
  float                 calculeF(const vector<float> & x) const;
  float                 calculeF_Untransformed(const vector<float> & x) const;
  // same thing, but we don't recompute phi(x)
  float                 calculeF(const vector<float> & x, const vector<float> & phi) const;
  float                 calculeF_Untransformed(const vector<float> & x, const vector<float> & phi) const;
  // compute phi(x)
  bool                  calculePhi(const vector<float> & x, vector<float> & phi) const;
  ...
  void                  setDontCutVar(string Name, bool Value);
  bool                  getDontCutVar(string Name) const;
  ...
  void                  setCustomArguments(string Arguments);
  string                getCustomArguments() const;
  void                  setCustomReturn_Before(string Arguments);
  string                getCustomReturn_Before() const;
  void                  setCustomReturn_After(string Arguments);
  string                getCustomReturn_After() const;
 private:
  int                   _nbMaxPartitions;
  float                 _residuGapPercentage;

  vector<LL_Dimension*> _dimensionSet;
  vector<LL_Mesure*>    _mesureSet;
  vector<LL_Partition*> _partitionSet;
  vector<float>         _Weight;

  vector<LL_Dimension*> stack_dimensionSet;
  vector<LL_Mesure*>    stack_mesureSet;
  vector<LL_Partition*> stack_partitionSet;
  float                 stack_residu;

  vector<LL_Dimension*> stack_good_dimensionSet;
  vector<LL_Mesure*>    stack_good_mesureSet;
  vector<LL_Partition*> stack_good_partitionSet;
  float                 stack_good_residu;

  vector<float>         _paramList;
  ...
  vector<bool>   ListOfVarToDontCut;
  bool   _exportTypeDouble;
  string _customArguments;
  string _customReturn_Before;
  string _customReturn_After;
};
#endif
