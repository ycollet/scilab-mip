...
public:
  ...
  vector<float> getEcartTypeSet()   const {return _ecartTypeSet;}
  ...
  float         calculeZ(const vector<float> & x) const;
  ...
  float         calculeValeurSelonRegression(const vector<float> & x) const;
 ...
 private:
  vector<float> _dimensionMinSet;
  vector<float> _dimensionMaxSet;
  vector<float> _coeffSet;
  vector<bool>  _freezeCoeffSet;
  vector<bool>  _inhibeSet;
  float         _coeff0;
  bool          _freezeCoeff0;
  float         _residu;
  bool          _DoNotCut;
