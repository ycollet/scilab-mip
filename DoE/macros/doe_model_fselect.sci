function [model_new, coeff_new] = doe_model_fselect(nb_var, model_old, measures, Log)
...
for l=1:nb_var
  for i=1:nb_var
    ...
    if (Log) then
      printf('doe_model_fselect: Residual = %f / %f ', Residual, Max_Residual);
  ...
  if ((Log)&(Max_Pos~=-1)) then
    printf('doe_model_fselect: Variable %d selected. Residual = %f\n', Max_Pos, Max_Residual);
  end
