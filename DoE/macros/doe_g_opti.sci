function [M_doe, history] = doe_g_opti(M_init, M_cand, doe_size, model, l_bounds, u_bounds, ItMX, p_level, Log, size_tabu_list)

[nargout, nargin] = argn();

if (size(M_cand,1)<=doe_size) then
  error("doe_g_opti: candidate set too small");
end
...
if (~isdef('model','local')) then
  error("doe_g_opti: error, model is mandatory");
end
...
Value       = comp_g_opti_crit(M_doe,model);
MaxValue    = Value;
OldValue    = Value;
Permutation = -1;
...
while((Progress>=p_level)&(Iter<ItMX))
  ...
  for i=max([size(M_init,1),1]):doe_size
    for j=1:size(M_cand,1)
      if (~isempty(find(Tabu_List==j))) then
        if (Log) then
          printf('doe_g_opti: Tabu list used\n');
        end
        continue;
      end
      ...
      if (abs(cond(H'*H))<1e-4) then
        ...
        M_cand(j,:) = Aux;
        if (Log) then
          printf('doe_g_opti: singular permutation skipped\n');
        end
        continue;
      end
      ...
      if (Value>MaxValue) then
        if (Log) then
          printf('doe_g_opti: G crit = %f / MaxValue = %f / Permutation = %d / Point = %d / %d\n', Value, MaxValue,j, i, doe_size);
        end
        MaxValue    = Value;
        Permutation = j;
      end
    ...
    if (Permutation~=-1) then
      ...
      if (Log) then
        printf('doe_g_opti: Good permutation found between points %d and %d\n', i, Permutation);
      end
      ...
      if (Log) then
        printf('doe_g_opti: No good permutation found for point %d\n', i);
      end
    end
  end
  ...
  if (Log) then
    printf('doe_g_opti: Iteration %d / %d - progress = %f / MaxValue = %f / OldValue = %f\n', Iter, ItMX, Progress, MaxValue, OldValue);
  end
