...
for k = 1:K
  ...
  [sm, s_idx] = lex_sort(s,1,'unique');
  b_interpolated = interp1(s(s_idx), b(s_idx, :), (0:step:1)');
  res(k,:) = sum((ytest*ones(1,steps) - Xtest*b_interpolated').^2,'r');
end
...
idx_opt2 = find(res_mean < limit);
if (~isempty(idx_opt2)) then
  idx_opt = idx_opt2;
end
...
t = sum(abs(b),'c');
s = (t - min(t))/max(t - min(t));
[sm, s_idx] = unique(gsort(s,'lr','i'));
b_opt = interp1(s(s_idx), b(s_idx,:), s_opt);
endfunction
