...
*/
  ...
  if (first) 
      ...
      if (lrnd && c == a) 
	{
	 ...
	 by powering.  So we find t as the smallest positive integer for which   
	 fl( beta**t + 1.0 ) = 1.0. */
...
*/
  ...
  extern /* Subroutine */ int dlamc1_(integer *, integer *, logical *, logical *);
  extern doublereal dlamc3_(doublereal *, doublereal *);
  static logical lieee1;
  ...
  if (first) 
	...
	{
	  if ((i__1 = ngpmin - ngnmin, d_abs(i__1)) == 1) 
	    {
      ...
      else if ((i__1 = ngpmin - ngnmin, d_abs(i__1)) == 1 && gpmin == gnmin)
	{
