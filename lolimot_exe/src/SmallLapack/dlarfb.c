	...
	*(unsigned char *)transt = 'N';
	...
	if (lsame_(direct, "F")) 
	...
	else 
	  {
		...
		for (j = 1; j <= i__1; ++j) 
		  {
		    dcopy_(n, &c___ref(*m - *k + j, 1), ldc, &work_ref(1, j), &c__1);
		    /* L70: */
		  }
		...
		for (j = 1; j <= i__1; ++j) 
		  {
		    dcopy_(m, &c___ref(1, *n - *k + j), &c__1, &work_ref(1, j), &c__1);
		    /* L100: */
		  }
	...
	if (lsame_(direct, "F")) 
		...
		if (*n > *k) 
		  {
		    ...
		    dgemm_("No transpose", "No transpose", m, &i__1, k, &
			   c_b25, &work[work_offset], ldwork, &v_ref(1, *k + 1),
			   ldv, &c_b14, &c___ref(1, *k + 1), ldc);
		  }
	...
	else 
	  {
		...
		/* W := W * V2' */
		dtrmm_("Right", "Lower", "Transpose", "Unit", n, k, &c_b14, &v_ref(1, *m - *k + 1),
		...
		if (*m > *k) 
		  {
		    /* C1 := C1 - V1' * W' */
		    i__1 = *m - *k;
		    dgemm_("Transpose", "Transpose", &i__1, n, k, &c_b25, &v[v_offset], ldv, &work[work_offset], ldwork, &
			   c_b14, &c__[c_offset], ldc);
		  }
		...
		/* W := W * V2' */
		dtrmm_("Right", "Lower", "Transpose", "Unit", m, k, &c_b14, &v_ref(1, *n - *k + 1), 
		       ldv, &work[work_offset], ldwork);
		if (*n > *k) 
		  {
		    /* W := W + C1 * V1' */
