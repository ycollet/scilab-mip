...
L40:
	...
	if (s <= t) 
	  {
	    doublereal AuxRes = s / t + 1.;
	    s = z__[nn - 3] * (z__[nn - 5] / (t * (d_sqrt(&AuxRes) + 1.)));
	  } 
	else 
	  {
	    doublereal AuxRes = t + s;
	    s = z__[nn - 3] * (z__[nn - 5] / (t + d_sqrt(&t) * d_sqrt(&AuxRes)));
	  }
...
/*           Success. */
		...
		tol * (*sigma + dn1) && d_abs(dn) < tol * *sigma) {
