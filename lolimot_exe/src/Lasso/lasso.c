...
static void lasso_alloc(int n, int m);
...
static double *xtr=NULL, *btmp=NULL, *qtr=NULL, *rinvt_theta=NULL, *step=NULL, ytyd2=0.0;
...
// Paramètres de lasso
// x  : matrice de regression [nxp]
// pn : nombre de points     [n]
// pm : nombre de paramètres [p]
// pt : bound - limite sur une distance L1 (0.5 par défaut)
// beta  : vecteur paramètres initiaux du modele linéaire [p]
// y     : vecteur de mesures [n]
// yhat1 : vecteur des estimations [n]
// r     : residu [n]
// lagrangian : coeff de penalisation (1 par défaut)
// psuc    : success 0 si tout se passe bien <0 sinon
// pverb   : trace 1 ou 0
// pas_sub : 1 ou 0 (0 par défaut - 1 pour une utilisation dans R)

// Paramètres de mult_lasso
// x  : matrice de regression [nxp]
// pn : nombre de points     [n]
// pm : nombre de paramètres [p]
// pt : bound - limite sur une distance L1 (les valeurs de bornes doivent être ordonnées) [m]
// pl : nombre de problème à résoudre [m]
// beta  : vecteur paramètres initiaux du modele linéaire [p]
// y     : vecteur de mesures [n]
// yhat1 : vecteur des estimations [n][m]
// r     : residu [n][m]
// lagrangian : coeff de penalisation (1 par défaut) [m]
// psuc    : success 0 si tout se passe bien <0 sinon
// pverb   : trace 1 ou 0
// pas_sub : 1 ou 0 (0 par défaut - 1 pour une utilisation dans R)
           ...
           double *lagrangian, int *psuc, int *pverb, int *pas_sub)
{
    ...
    double   t = *pt, prec;
    int      n = *pn, m = *pm, verb = *pverb, as_sub = *pas_sub;
    int      not_solved;
    ...
    if( as_sub ) 
      {
	b_1norm = 0.0;
	for(j=0;j < num_nz_x;j++) b_1norm += fabs(beta[nz_x[j]]);
      }
    else 
      {
	b_1norm = 0.0;
	...
	if(verb) 
	  {
	    printf("******************************\n");
	    printf("Rescaling beta from L1-norm %f to %f\n", b_1norm, t);
	  }

	for(j=0; j < num_nz_x; j++) beta[nz_x[j]] = beta[nz_x[j]] * t/b_1norm;
	b_1norm = t;
      }

    for(i=0; i <  n; i++) yhat1[i] = 0.0;
    ...
    *psuc=0;
	...
	printf("******************************\n");
	printf("\nIteration number: %d\n", num_iter);
	printf("Value of primal object function      : %f\n", p_obj);
	printf("Value of dual   object function      : %f\n", d_obj);
	printf("L1 norm of current beta              : %f", b_1norm);
	printf(" <= %f\n", t);
	printf("Maximal absolute value in t(X)%%*%%r   : %e", max_val);
	printf(" attained %d time(s)\n", max_num);
	printf("Number of parameters allowed to vary : %d\n", num_nz_x);
	num_iter++;
      }
	...
	do
	  {
	    ...
	    for(j=0;j < num_nz_x;j++) 
	      {
		tmp = 0.0;
		for(i=0;i < n;i++,q_elem++) tmp += *q_elem*r[i];
		qtr[j] = tmp;
	      }

	    if( b_1norm < (1.0-prec)*t ) mu = 0.0;
	    else 
	      {
		...
		for(j=0;j < num_nz_x;j++,r_elem++) 
		  {
		    tmp = theta[j];
		    for(k=0;k < j;k++,r_elem++) tmp -= *r_elem * rinvt_theta[k];
	    ...
	    if(mu<=0.0) for(j=0;j < num_nz_x;j++) step[j] = qtr[j];
	    else        for(j=0;j < num_nz_x;j++) step[j] = qtr[j] - mu*rinvt_theta[j];

	    /* h=R^{-1}z can be calculated by solving Rh=z */
	    for(j=num_nz_x-1;j>=0;j--)
	      {
		tmp = step[j];
		for(k=num_nz_x-1;k>j;k--) tmp -= RMAT(j,k) * step[k];
		step[j] = tmp /  RMAT(j,j);
	      }
		...
		if(b_1norm_old < (1.0-prec)*t) 
		  {
		    if(verb) printf("  -->\tStepping onto the border of the L1 ball.\n");
		    rho_up  = rho = 1.0;
		    rho_low = 0.0;
			...
			for(j=0; j < num_nz_x; j++) beta[nz_x[j]] = btmp[j]+rho*step[j];
		    ...
		    for(j=0;j < num_nz_x;j++) 
		      {
			if(fabs(beta[nz_x[j]]) < prec) theta[j] = btmp[j] > 0 ? -1 : 1;
			else                           theta[j] = beta[nz_x[j]] < 0 ? -1 : 1;
		      }
		  }
		else 
		  {
			...
			*psuc= -1;
		    ...
		    if(verb) printf("  -->\tRemoving variable: %d",nz_x[to_del]+1);
		    ...
		    if(verb) 
		      {
			if(b_1norm < (1-prec)*t) printf(", and stepping into the interior of the L1 ball\n");
			else                     printf("\n");
		      }
		  }
	      }
	...
	if(verb) printf("  -->\tAdding variable: %d\n",max_ind+1);
    }
EXIT_HERE:
    if( !as_sub) lasso_free();
}
...
{
  double prec;
  int n = *pn, m = *pm, l = *pl, verb = *pverb, as_sub = TRUE;
  int i, j;
  ...
  *psuc = 0;
  ...
  for(i=0; i<l; i++) 
	...
	{
	  printf("\n\n++++++++++++++++++++++++++++++\n");
	  printf("Solving problem number %d with bound %f\n", i+1, pt[i]);
	  printf("++++++++++++++++++++++++++++++\n");
	}
...
static void lasso_alloc(int n, int m)
{
  ...
  if( nz_x != NULL || theta != NULL || xtr != NULL || btmp != NULL ||
    ...
    {
      fprintf(stderr,"Possible memory corruption or memory leak.\n");
      lasso_free();
    }

  nz_x = Calloc(m,int);
  if( nz_x == NULL ) fprintf(stderr, "lasso_alloc: cannot allocate memory.\n");
  theta = Calloc(m,int);
  if( theta==NULL ) fprintf(stderr, "lasso_alloc: cannot allocate memory.\n");
  xtr = Calloc(m,double);
  if( xtr==NULL ) fprintf(stderr, "lasso_alloc: cannot allocate memory.\n");
  btmp = Calloc(m,double);
  if( btmp==NULL ) fprintf(stderr, "lasso_alloc: cannot allocate memory.\n");
  qtr = Calloc(m,double);
  if( qtr==NULL ) fprintf(stderr, "lasso_alloc: cannot allocate memory.\n");
  rinvt_theta = Calloc(m,double);
  if( rinvt_theta==NULL ) fprintf(stderr, "lasso_alloc: cannot allocate memory.\n");
  step = Calloc(m,double);
  if( step==NULL ) fprintf(stderr, "lasso_alloc: cannot allocate memory.\n");
  qr_init(n);
}
...
static void qr_init(int n)
{
  ...
  if(qmat==NULL) fprintf(stderr, "qr_init: cannot allocate memory.\n");

  rmat = Calloc(qr_max_size*(qr_max_size+1)/2,double);
  if(rmat==NULL) fprintf(stderr, "qr_init: cannot allocate memory.\n");
}
...
static void qr_incr(void) 
{
  ...
  if(rmat==NULL) fprintf(stderr, "qr_incr: cannot allocate memory.\n");
  ...
  if( qr_max_size >= q_nrow) 
      ...
      if( qr_max_size-QR_CHUNK < q_nrow) 
	{
	  ...
	  if(qmat==NULL) fprintf(stderr, "qr_incr: cannot allocate memory.\n");
	}
    } 
  else 
    {
...
static void qr_del(int l, int aug) 
{
    ...
    if( l<0 || l>=r_ncol ) fprintf(stderr, "qr_del: Invalid column number\n");
    r_ncol--;
    if(l == r_ncol)
      {
	if( aug ) fprintf(stderr, "qr_del: Trying to delete last column of augmented matrix\n");
	else      return;
      }
      ...
      if( l < q_nrow ) 
	{
	  ...
	  for(i=l0; i<r_ncol; i++, col_k +=i) Memcpy(col_k,col_k+i+1,q_nrow);
	  ...
	  if( *a < 0 ) 
	    {
	      ...
	      for(j=l0;j<r_ncol;j++,a+=j) *a = - *a;
	      col_k = qmat+(q_nrow-1)*q_nrow;
	      for(i=0;i<q_nrow;i++,col_k++) *col_k = -*col_k;
	    }
	}
      else
	{ /* just shift last columns to the left */
	  l0 = l;
	  col_k   = RCOL(l0); /* first element in column $l_0$ in R */
	  for(i=l0; i<r_ncol; i++, col_k +=i) Memcpy(col_k,col_k+i+1,q_nrow);
...
static void qr_add(double *x, int swap) 
{
	...
	for(i=0;i<q_nrow;i++) 
	  {
	    tmp = 0.0;
	    for(j=0;j<q_nrow;j++,q_elem++) tmp += *q_elem * x[j];
	    r_new_col[i] = tmp;
	  }
	if( swap )
	  { /* swap last two columns */
		...
		*col_l   = - *col_l;
		...
		for(j=0;j<q_nrow;j++) q_new_col[j] = -q_new_col[j];
	      }
	  }
	r_ncol++;
	return;
	...
	for(j=0; j<r_ncol;j++) 
	  {
	    tmp = 0.0;
	    for(i=0;i<q_nrow;i++,q_elem++) tmp += *q_elem * q_new_col[i];
	    r_new_col[j] += tmp;
	    q_elem -= q_nrow;
	    for(i=0;i<q_nrow;i++,q_elem++) q_new_col[i] -= *q_elem*tmp;
	  }
	tmp = 0.0;
	...
	if( norm_new > 0.1*norm_init*DBL_EPSILON ) norm_last = norm_new;
	else
	  {
	    norm_init = norm_last = 0.1*norm_last*DBL_EPSILON;
	    for(i=0;i<q_nrow;i++) q_new_col[i] = 0.0;
	    if(q_use_row == q_nrow) fprintf(stderr, "qr_add: Cannot orthogonalise new column\n");
	    q_new_col[q_use_row] = norm_new = norm_last;
	    q_use_row++;
	  }
      }
    
    for(i=0;i<q_nrow;i++) q_new_col[i] /= norm_new;
    for(i=0;i<r_ncol;i++) r_new_col[i] *= norm_orig;
