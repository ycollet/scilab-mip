  ...
  *info = 0;
  ...
  if (nb >= nbmin && nb < k && nx < k) 
      ...
      for (i__ = 1; i__2 < 0 ? i__ >= i__1 : i__ <= i__1; i__ += i__2) 
	{
	  ...
	  i__3 = *n - i__ + 1;
	  dgelq2_(&ib, &i__3, &a_ref(i__, i__), lda, &tau[i__], &work[1], &iinfo);
	  if (i__ + ib <= *m)
	    {
	      /* Form the triangular factor of the block reflector   
		 H = H(i) H(i+1) . . . H(i+ib-1) */
	      i__3 = *n - i__ + 1;
	      dlarft_("Forward", "Rowwise", &i__3, &ib, &a_ref(i__, i__), lda, &tau[i__], &work[1], &ldwork);
  ...
  if (i__ <= k) 
    {
      i__2 = *m - i__ + 1;
      i__1 = *n - i__ + 1;
      dgelq2_(&i__2, &i__1, &a_ref(i__, i__), lda, &tau[i__], &work[1], &iinfo);
    }
