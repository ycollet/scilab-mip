...
c  documented below.
      ...
      subroutine ds_start_up(n,nm,nmi,a,la,nk,e,ls,aa,ll,mode,ifail)
...
c     write(nout,*)'ls',(ls(ij),ij=nm+1,nmi)
      if(mode.ge.3)then
        call ds_re_factor(n,nm,a,la,aa,aa(ns1),aa(nt1),ll,ll(lc1),
     * ll(li1))
        call ds_check_L(n,aa,ifail)
        if(ifail.eq.1)then
          mode=2
          goto1
        endif
...
c  set a lower bound on e(i)
        emin=1.D0
        do i=1,nmi-n
          emin=max(emin,ds_ailen(n,a,la,i))
        enddo
        emin=1.D0/emin
      endif
      do i=1,n
...
c  shift designated bounds to end
    ...
    2 continue
        ...
        q=abs(ls(j))
c  extend factors
      call ds_aqsol(n,a,la,q,aa,aa(nt1),aa(mx1),aa,ll,ll(lc1),ll(li1))
        m1p=m1+1
        call ds_linf(nn-m1,aa(nt+m1p),z,iz)
        iz=iz+m1
        if(z.le.tol)then
...
c  pivot interchange
          ll(li+ll(m1p))=iz
          call ds_iexch(ll(m1p),ll(iz))
          call ds_rexch(aa(nt+m1p),aa(nt+iz))
          ll(li+ll(m1p))=m1p
        endif
        p=ll(m1p)
        tp=aa(nt+m1p)
      call ds_eptsol(n,a,la,p,a,aa,aa(ns1),aa(nt1),ll,ll(lc1),ll(li1))
        aa(ns+m1p)=1.D0
...
c       eq=ep/tp
        ...
        tp_=tp/ep
        tpsq=tp_**2
        call ds_aqsol(n,a,la,-1,a,aa(nu1),aa(mx1),aa,ll,ll(lc1),
     * ll(li1))
        do i=1,m1p
          aa(nu+i)=aa(ns+i)/ep
        enddo
...
c     call ds_checkout(n,a,la,aa,ll,ll(lc1),ll(li1))
      mp=-1
      mq=-1
      ifail=0
      return
      ...
      subroutine ds_refactor(n,nm,a,la,aa,ll,ifail)
...
c     write(nout,*)'refactor'
     ...
     * ll(li1))
      ...
      subroutine ds_pivot(p,q,n,nm,a,la,e,aa,ll,ifail,info)
      implicit double precision (a-h,r-z), integer (i-q)
...
c     write(nout,*)'pivot: p,q =',p,q
      ifail=0
      if(p.ne.mp)then
      call ds_eptsol(n,a,la,p,a,aa,aa(ns1),aa(nt1),ll,ll(lc1),
     * ll(li1))
        e(p)=sqrt(ds_scpr(0.D0,aa(ns1),aa(ns1),m1+1))
        mp=p
      endif
      if(q.ne.mq)then
        call ds_aqsol(n,a,la,q,a,aa(nt1),aa(mx1),aa,ll,ll(lc1),ll(li1))
        mq=q
      endif
c  update steepest edge coefficients
      tp=aa(nt+ll(li+p))
...
c     eq=ep/tp
      ...
      do i=m1+2,n
        aa(nu+i)=0.D0
      enddo
      call ds_aqsol(n,a,la,-1,a,aa(nu1),aa(mx1),aa,ll,ll(lc1),ll(li1))
c     write(nout,*)'row perm',(ll(ij),ij=1,n)
c     write(nout,*)'column perm',(ll(lc+ij),ij=1,m1)
...
c             write(nout,*)'m0 + m1 = mxm1:  re-centre triangle'
        ...
        call ds_c_flma(n,a,la,p,aa,ll,ll(lc1),ll(li1))
    1   continue
        if(q.le.n)then
          call ds_r_flma(n,a,la,q,aa,ll,ll(lc1),ll(li1))
        else
          m1=m1+1
          mm=mm+m0+m1
          aa(mm)=1.D0
          aa(mm)=ds_aiscpri1(n,a,la,q-n,aa(mm-m1+1),0.D0,ll,ll(li1),m1)
...
c         call ds_eptsol(n,a,la,i,a,aa,aa(ns1),aa(nt1),ll,ll(lc1),ll(li1))
c         ei=sqrt(ds_scpr(0.D0,aa(ns1),aa(ns1),n))
...
c    *  write(nout,*)'error in steepest edge coefficients =',emax
      ...
      subroutine ds_fbsub(n,jmin,jmax,a,la,q,b,x,ls,aa,ll,save)
      implicit double precision (a-h,r-z), integer (i-q)
      logical save
...
c     write(nout,*)'fbsub  q =',q
      if(save)then
        if(q.ne.mq)then
        call ds_aqsol(n,a,la,q,b,aa(nt1),aa(mx1),aa,ll,ll(lc1),ll(li1))
          mq=q
        endif
        do j=jmin,jmax
          i=abs(ls(j))
        ...
        call ds_aqsol(n,a,la,q,b,aa(nu1),aa(mx1),aa,ll,ll(lc1),ll(li1))
        do j=jmin,jmax
          i=abs(ls(j))
          x(i)=aa(nu+ll(li+i))
        enddo
      ...
      subroutine ds_tfbsub(n,a,la,p,b,x,aa,ll,ep,save)
      implicit double precision (a-h,r-z), integer (i-q)
      logical save
...
c     write(nout,*)'tfbsub  p =',p
      if(save)then
        if(p.ne.mp)then
      call ds_eptsol(n,a,la,p,b,aa,aa(ns1),aa(nt1),ll,ll(lc1),ll(li1))
          mp=p
        endif
        do i=1,n
          x(ll(i))=aa(ns+i)
        enddo
        if(p.gt.0)ep=sqrt(ds_scpr(0.D0,aa(ns1),aa(ns1),m1+1))
      else
      call ds_eptsol(n,a,la,p,b,aa,aa(nu1),aa(nt1),ll,ll(lc1),ll(li1))
        do i=1,n
          x(ll(i))=aa(nu+i)
        enddo
      endif
...
c******** The following routines are internal to denseL.f **************
      ...
      subroutine ds_re_factor(n,nm,a,la,T,sn,tn,lr,lc,li)
      implicit double precision (a-h,r-z), integer (i-q)
...
c     write(nout,*)'column perm',(lc(ij),ij=1,m1)
      ...
      do i=1,m1-1
        ...
        if(q.le.0)goto1
c  form L.a_q
        call ds_iscatter(a,la,q,li,sn,n)
c       write(nout,*)'aq =',(sn(ij),ij=1,m1)
        jj=mm
        j1=i1
        do j=i,m1
          tn(j)=ds_scpr(sn(j),T(j1),sn,im)
          j1=jj+m0+1
          jj=j1+j
        enddo
        call ds_iunscatter(a,la,q,li,sn,n)
c       write(nout,*)'L.aq =',(tn(ij),ij=i,m1)
        call ds_linf(m1-im,tn(i),z,iz)
        if(iz.gt.1)then
c  pivot interchange
          iz=iz-1
          call ds_vexch(T(i1),T(i1+iz*(m0+i)+iz*(iz-1)/2),im)
          iz=iz+i
          call ds_rexch(tn(i),tn(iz))
          li(lr(i))=iz
          call ds_iexch(lr(i),lr(iz))
          li(lr(i))=i
        endif
...
c  update L
        ...
        do j=i+1,m1
          z=tn(j)/zz
          call ds_mysaxpy(z,T(i1),T(j1),i-1)
          T(j1+im)=z
c         write(nout,*)'L(j) =',(T(ij),ij=j1,j1+im)
      ...
      if(q.le.0)goto1
      call ds_iscatter(a,la,q,li,sn,n)
      T(mm)=ds_scpr(sn(m1),T(mm-m1+1),sn,m1-1)
...
c     call checkout(n,a,la,T,lr,lc1,li)
    ...
    1 continue
      ...
      subroutine ds_check_L(n,T,ifail)
      implicit double precision (a-h,r-z), integer (i-q)
      dimension T(*)
      common/noutc/nout
      common/factorc/m0,m1,mm0,mm,mp,mq
...
c     write(nout,*)'dmin =',dmin
      ...
      subroutine ds_aqsol(n,a,la,q,b,tn,xm,T,lr,lc,li)
      implicit double precision (a-h,r-z), integer (i-q)
...
c         call ds_isaipy(1.D0,a,la,q-n,tn,n,lr,li)
          call ds_iscatter(a,la,q-n,li,tn,n)
        endif
      elseif(q.eq.0)then
        do i=1,n
          tn(li(i))=b(i)
...
c     write(nout,*)'tn =',(tn(i),i=1,n)
      ii=mm
      do i=m1,1,-1
        xm(i)=(ds_scpr(tn(i),T(ii-i+1),tn,i-1))/T(ii)
        call ds_isaipy(-xm(i),a,la,lc(i)-n,tn,n,lr,li)
        ii=ii-m0-i
      enddo
      do i=1,m1
        tn(i)=xm(i)
...
c     write(nout,*)'tn =',(tn(i),i=1,n)
      ...
      subroutine ds_eptsol(n,a,la,p,b,T,sn,tn,lr,lc,li)
      implicit double precision (a-h,r-z), integer (i-q)
...
c     endif
      if(p.gt.n)then
        ...
        if(pr.ne.m1)then
          z=tn(pr)
          call ds_r_shift(tn(pr),m1-pr,1)
          tn(m1)=z
          call ds_c_flma(n,a,la,p,T,lr,lc,li)
          m1=m1+1
          mm=mm+m0+m1
          li(p)=m1
          lc(m1)=p
          T(mm)=1.D0
          T(mm)=ds_aiscpri1(n,a,la,p-n,T(mm-m1+1),0.D0,lr,li,m1)
          if(T(mm).eq.0.D0)T(mm)=eps
c         write(nout,*)'PAQ factors'
c         ij=m0+mm0
c         do ii=1,m1
...
c         call ds_checkout(n,a,la,T,lr,lc,li)
        endif
        ii=mm-m1
        z=1.D0/T(mm)
        do i=1,m1-1
        ...
        if(p.eq.0)then
          ...
          do i=1,m1
            ii=ii+i
            j=lc(i)
            sn(i)=-ds_aiscpri(n,a,la,j-n,sn,-b(j),lr,li)/T(ii)
            call ds_mysaxpy(sn(i),T(ij),sn,i-1)
            ii=ii+m0
            ij=ii+1
          enddo
        else
          ...
          if(pr.le.m1)goto1
          m1p=m1+1
          call ds_iexch(lr(pr),lr(m1p))
          call ds_iexch(li(lr(pr)),li(lr(m1p)))
          call ds_rexch(tn(pr),tn(m1p))
          do i=1,n
            sn(i)=0.D0
          enddo
          sn(m1p)=1.D0
          do i=1,m1
            ii=ii+i
            sn(i)=-ds_aiscpri(n,a,la,lc(i)-n,sn,0.D0,lr,li)/T(ii)
            call ds_mysaxpy(sn(i),T(ij),sn,i-1)
            ii=ii+m0
            ij=ii+1
          enddo
...
c     write(nout,*)'sn =',(sn(i),i=1,n)
    ...
    1 continue
      ...
      subroutine ds_c_flma(n,a,la,q,T,lr,lc,li)
      implicit double precision (a-h,r-z), integer (i-q)
...
c     write(nout,*)'c_flma: q =',q
      ...
      if(q.gt.n)then
        ...
        if(qc.le.m1)goto1
        call ds_iexch(lr(qc),lr(m1+1))
        call ds_iexch(li(lr(qc)),li(lr(m1+1)))
        call ds_ishift(lr(2),m1,-1)
        lr(1)=q
        do i=1,m1+1
          li(lr(i))=i
        enddo
...
c         write(nout,*)'m0 = 0:  re-centre triangle'
      ...
      do i=qc,m1
        ...
        T(ii)=1.D0
        u21=T(iip)
        u11=ds_aiscpri1(n,a,la,lc(i)-n,T(ii1-im),0.D0,lr,li,i)
        ij=ii+im-iswap
c       write(nout,*)'i,im,ii,iip,iswap,ij',i,im,ii,iip,iswap,ij
        l21=T(ij)
        if(abs(l21).le.eps)l21=0.D0
        if(iswap.gt.0)call ds_r_shift(T(ij),iswap,1)
        del=u21-l21*u11
...
c         else
            ...
            call ds_mysaxpy(r,T(ii1-im),T(ii1),i-1)
c         endif
          T(ii)=u11
          T(ii+im)=l21+r
          if(iswap.gt.0)then
          ...
          r=-u11/del
          if(abs(r).le.eps)r=0.D0
          call ds_permop(T(ii1-im),T(ii1),r,-l21,i-1)
          T(ii)=del
          T(ii+im)=r
          call ds_iexch(lr(i),lr(i+1))
          call ds_iexch(li(lr(i)),li(lr(i+1)))
          iswap=iswap+1
        endif
        ii=iip
      enddo
    ...
    1 continue
      ...
      subroutine ds_r_flma(n,a,la,p,T,lr,lc,li)
      implicit double precision (a-h,r-z), integer (i-q)
      dimension a(*),la(*),T(*),lr(*),lc(*),li(*)
      common/noutc/nout
      common/epsc/eps,tol,emin
...
c       write(nout,*)'new row =',(T(j),j=ii1,ii+im)
        if(abs(del).le.abs(l11)*max(abs(u11),abs(u12)))then
          ...
          r=-u12/del
          if(abs(r).le.eps)r=0.D0
          call ds_permop(T(ii1-im),T(ii1),r,l11,i)
          call ds_iexch(lc(i),lc(i+1))
          call ds_iexch(li(lc(i)),li(lc(i+1)))
          T(iip)=r
          u22=u11*u22/del
          u11=del
        endif
        call ds_r_shift(T(ip),i-pr,1)
        T(ii)=u11
        u11=u22
        ip=ip+im
        ii=iip
      enddo
      call ds_ishift(lr(pr),m1-pr+1,1)
      lr(m1+1)=p
      do j=pr,m1+1
        li(lr(j))=j
      enddo
c     if(T(ip).eq.0.D0)T(ip)=eps
      l11=-T(ip+m0+m1)/T(ip)
      call ds_saxpyx(l11,T(mm-m1+1),T(mm+m0+1),m1)
      call ds_r_shift(T(ip),m1-pr,1)
      T(mm)=l11*u11
      if(T(mm).eq.0.D0)T(mm)=eps
      return
      end

      subroutine ds_permop(v1,v2,r,s,n)
      implicit double precision (a-h,o-z)
      dimension v1(*),v2(*)
      common/noutc/nout
      if(s.eq.0)then
        if(r.eq.0)then
          call ds_vexch(v1,v2,n)
        else
          do i=1,n
            z=v2(i)
            v2(i)=v1(i)+r*z
      ...
      subroutine ds_checkout(n,a,la,T,lr,lc,li)
      implicit double precision (a-h,o-z)
      dimension a(*),la(*),T(*),lr(*),lc(*),li(*)
      common/noutc/nout
      common/mxm1c/mxm1
      ...
      do i=1,m1
        ...
        T(ii)=1.D0
        do j=1,i-1
          e=ds_aiscpri1(n,a,la,lc(j)-n,T(ii1),0.D0,lr,li,i)
          emax=max(emax,abs(e))
          gmax=max(gmax,abs(T(ii+m0+j)))
        enddo
        e=ds_aiscpri1(n,a,la,lc(i)-n,T(ii1),-d,lr,li,i)
        emax=max(emax,abs(e))
        T(ii)=d
      enddo
