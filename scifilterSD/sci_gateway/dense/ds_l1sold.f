c  Copyright (C) 2010 Roger Fletcher
      ...
      subroutine ds_l1sold(n,m,k,kmax,maxg,a,la,x,bl,bu,f,g,r,w,e,ls,
...
c     call ds_check1(n,lp(1),nm,k,kmax,g,a,la,x,bl,bu,r,ls,lp,ws(nb1),f,
c    *  ws,lws,cws,1,p,rp)

   21 continue
c  l1 optimality test
      call ds_optest1(peq,k,n,bl,bu,r,e,ls,rp,pj)
...
c     print 2,'slope for r(169) =',ds_aiscpr(n,a,la,169-n,ws(na1),0.D0)
c     print 2,'slope for r(217) =',ds_aiscpr(n,a,la,217-n,ws(na1),0.D0)
c     print *,'plus =',plus
...
c  pivot interchange
        ...
        call ds_pivot(p,q,n,nm,a,la,e,ws(lu1),lws(ll1),ifail,npv)
        if(ifail.ge.1)then
        ...
        elseif(r(p).gt.0.D0)then
          call ds_saipy(-sign(1.D0,dble(ls(pj))),a,la,p-n,g,n)
          call ds_newg
          r(p)=-alpha
        else
          rpu=bu(p)-bl(p)-alpha
...
c1001 format(a/(i3,1x,e14.8,3(i4,1x,e14.8)))
      ...
      subroutine ds_optest1(peq,k,n,bl,bu,r,e,ls,rp,pj)
      implicit double precision (a-h,r-z), integer (i-q)
...
c  additional test for free variables
      ...
      subroutine ds_check1(n,nm,nmi,k,kmax,g,a,la,x,bl,bu,r,ls,lp,an,f,
...
c     if(e.gt.tol)stop
     ...
     *    '   f(x) =',ff
        if(e.gt.tol*max(1.D0,abs(f))) then
          ifail=100
          return
        endif
      endif  
...
c       print 2,'check err =',err
      ...
      do j=n+1,nm
        ...
        if(r(i).lt.0.D0)then
          if(i.gt.n)then
            call ds_saipy(-sign(1.D0,dble(ls(j))),a,la,i-n,an,n)
          else
            an(i)=an(i)-sign(1.D0,dble(ls(j)))
          endif
        endif
      enddo
      gnm=sqrt(ds_scpr(0.D0,an,an,n))
      e=0.D0
      do j=1,n
c       write(nout,*)'an =',(an(i),i=1,n)
        ...
        if(i.le.n)then
          ...
          call ds_saipy(-s*r(i),a,la,i-n,an,n)
          if(ls(j).gt.0)then
            s=ds_aiscpr(n,a,la,i-n,x,-bl(i))
          else
            s=-ds_aiscpr(n,a,la,i-n,x,-bu(i))
          endif
        endif
        if(abs(s).gt.e)then
          e=abs(s)
