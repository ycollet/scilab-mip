      ...
      subroutine ds_filterSD(n,m,x,al,f,fmin,cstype,bl,bu,ws,lws,v,nv,
     *  maxa,maxla,maxu,maxiu,kmax,maxg,rho,htol,rgtol,maxit,iprint,
     *  nout,ifail,functions,gradients)
...
c  scatter ws(nx.. and ws(nal.. and bound multipliers into x and al
    ...
    1 format(A,15I5)
      ...
      subroutine ds_filter_SD(n,f,fmin,cstype,bl,bu,ws,lws,v,nv,
     *  maxa,kmax,maxg,
...
c     if(itn.eq.164)iii=1
      call ds_glcpd(n,m,k,kmax,maxg,ws(last1),lws(nla1),d,dl,du,phi,
     *  -ainfty,g,r,w,e,ls,alp,lp,mlp,ipeq,ws,lws,cstype,v,nv,rgtol,
     *  mode,ifail,infty,iii,0,ds_funct,ds_grad,functions,gradients)
...
c     print *,'LP is infeasible: solve l1 subproblem',itn
      iii=0
c     if(itn.eq.9)iii=2
      call ds_l1sold(n,m,k,kmax,maxg,ws(last1),lws(nla1),d,dl,du,phi,
     *  g,r,w,e,ls,alp,lp,mlp,ipeq,ws,lws,cstype,v,nv,rgtol,ifail,iii,0)
c     print 4,'d1 =',(d(i),i=1,n)
c     print 4,'r =',(r(i),i=1,nm)
c     print 3,'ls =',(ls(i),i=1,nm)
      if(ifail.ne.0)then
        if(iprint.gt.1)print *,'unexpected fail in l1 subproblem'
        goto99
      endif
      if(abs(iph).eq.2)then
        call ds_addfil(h,f,filh,filf,1,nfil,mxf,ifail)
...
c       call ds_addfil(hJt,hJ,filh,filf,nfil1,nfil,mxf,ifail)
c       if(ifail.gt.0)return
        call ds_testfil(hJt,hJ,filh,filf,nfil1,nfil,ifail)
        if(ifail.eq.1)then
...
c     print 4,'al =',(ws(i),i=nal1,nal+m)
      do i=1,m
        call ds_saipy(ws(nal+i),ws(last1),lws(nla1),i,ws(naal1),n)
      enddo
   20 continue
...
c  filter test for LCP solution
      call ds_testfil(hxdJt,hxdJ,filh,filf,nfil1,nfil,ifail)
      if(ifail.eq.0)call ds_testfil(hxdJt,hxdJ,hJt,hJ,1,1,ifail)
...
c       if(itn.eq.68)iii=1
        call ds_glcpd(n,m,k,kmax,maxg,ws(next1),lws(nla1),d,dl,du,phi,
     *    0.D0,g,r,w,e,ls,alp,lp,mlp,ipeq,ws,lws,cstype,v,nv,rgtol,
     *    mode,ifail,mxgr,iii,0,ds_funct,ds_grad,functions,gradients)
c       print 4,'d =',(d(i),i=1,n)
c       print 4,'r =',(r(i),i=1,nm)
c       print 3,'ls =',(ls(i),i=1,nm)
...
c  accept LCP (iph=1) or projection (iph=-1) solution
      if(dq.lt.tol)then
        call ds_addfil(hJt,hJ,filh,filf,nfil1,nfil,mxf,ifail)
        if(ifail.gt.0)return
      endif
      do i=1,n
        ws(nx+i)=ws(nxd+i)
...
c     print *,'solve phase 2 LCP subproblem',itn
      iii=0
c     if(itn.eq.164)iii=1
      call ds_glcpd(n,m,k,kmax,maxg,ws(last1),lws(nla1),d,dl,du,phi,fmin,
...
c  filter test for LCP solution
      ...
      df=f-fxd
c     print 2,'dq,df',dq,df
      call ds_testfil(hxd,fxd,filh,filf,1,nfil,ifail)
      if(ifail.eq.0)call ds_testfil(hxd,fxd,h,f,1,1,ifail)
...
c     iph=2
   ...
   99 continue
     ...
     *    / 1.D20,  1.D4, 50,  50, 1000000/
      ...
      subroutine ds_testfil(h,f,filh,filf,nfil1,nfil,ifail)
      implicit double precision (a-h,o-z)
      dimension filh(*),filf(*)
...
c       if(hd.gt.filh(i).and.fp.gt.filf(i))then
      ...
      subroutine ds_addfil(h,f,filh,filf,nfil1,nfil,mxf,ifail)
      implicit double precision (a-h,o-z)
      dimension filh(*),filf(*)
      do i=nfil,nfil1,-1
      ...
      subroutine ds_funct(n,d,phi,ws,lws,cstype,functions,gradients)
      implicit double precision (a-h,o-z)
      dimension d(*),ws(*),lws(*)
      character cstype(*)
      external functions,gradients
...
c  projection subproblem
        phi=5.D-1*ds_scpr(0.D0,d,d,n)
        return
      elseif(iph.eq.0)then
c  LP subproblem
        phi=ds_aiscpr(n,ws(last1),lws(nla1),0,d,0.D0)
        return
      endif
      do i=1,n
        ws(nxd+i)=ws(nx+i)+d(i)
...
c     print *,'$cstype =',(cstype(i),i=1,m)
      if(iph.eq.2)then
        phi=ds_scpr(-ds_scpr(-fxd-alc,ws(nal1),ws(ncxd1),m),ws(naal1),
     * d,n)
      else
        phi=ds_scpr(-ds_scpr(-alc,ws(nal1),ws(ncxd1),m),ws(naal1),d,n)
        do i=1,m
          if(cstype(i).eq.'A')then
            phi=phi-ws(ncxd+i)
          elseif(cstype(i).eq.'Z')then
...
c     print 2,'$phi =',phi
      ...
      subroutine ds_grad(n,d,g,ws,lws,cstype,functions,gradients)
      implicit double precision (a-h,o-z)
     ...
     *  nal,nal1,naal,naal1,nxd,nxd1,ncx,ncx1,ncxd,ncxd1,nla1
      ...
      elseif(iph.eq.0)then
        do i=1,n
          g(i)=0.D0
        enddo
        call ds_saipy(1.D0,ws(last1),lws(nla1),0,g,n)
        return
      endif
c     print 4,'£x+d =',(ws(nxd+i),i=1,n)
      call gradients(n,m,ws(nxd1),ws(next1),ws,lws)
...
c     print 4,'£Ak.al =',(g(i),i=1,n)
      do i=1,m
        call ds_saipy(-ws(nal+i),ws(next1),lws(nla1),i,g,n)
      enddo
      if(iph.eq.2)then
        call ds_saipy(1.D0,ws(next1),lws(nla1),0,g,n)
      else
        do i=1,m
          if(cstype(i).eq.'A')then
            call ds_saipy(-1.D0,ws(next1),lws(nla1),i,g,n)
          elseif(cstype(i).eq.'Z')then
            call ds_saipy(1.D0,ws(next1),lws(nla1),i,g,n)
          endif
        enddo
