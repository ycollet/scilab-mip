...
c  CONSTITUTES RECIPIENT'S ACCEPTANCE OF THIS AGREEMENT
      ...
      subroutine ds_glcpd(n,m,k,kmax,maxg,a,la,x,bl,bu,f,fmin,g,r,w,e,
...
c  set up factors of basis matrix and permutation vectors
        ifail=mode
        call ds_start_up(n,nm,nmi,a,la,nk,e,ls,ws(lu1),lws(ll1),mode,
     * ifail)
...
c  refinement step loop
      ...
      enddo
      if(mode.gt.0)then
        call ds_warm_start(n,nm,a,la,x,bl,bu,r,ls,ws(lu1),
...
c  move inactive general c/s to the end
      do j=nm,n1,-1
        i=abs(ls(j))
        if(i.gt.n)then
          call ds_iexch(ls(j),ls(lp(1)))
          lp(1)=lp(1)-1
        endif
      enddo
      call ds_residuals(n,n1,lp(1),a,la,x,bl,bu,r,ls,f,g,ninf)
      if(ninf.gt.0)then
...
c  collect active equality c/s
      do j=1,n-k
        ...
        if(bu(i).eq.bl(i))then
          peq=peq+1
          call ds_iexch(ls(j),ls(peq))
        endif
      enddo
      call ds_residuals(n,lp(1)+1,nm,a,la,x,bl,bu,r,ls,f,g,ninf)
      lp(1)=nm
      if(ninf.gt.0)then
        gnorm=sqrt(ds_scpr(0.D0,g,g,n))
        gtol=sgnf*gnorm
        rgtol=max(rgtol,gtol)
        goto15
      endif
   10 continue
      ...
      n_inf=0
      call ds_funct(n,x,f,ws,lws,cws,functions,gradients)
      nfn=nfn+1
      if(f.lt.fmin)goto75
      call ds_grad(n,x,g,ws,lws,cws,functions,gradients)
      ngr=ngr+1
...
c     print 4,'gradient =',(g(i),i=1,n)
      do i=1,nm
        w(i)=0.D0
      enddo
      call ds_fbsub(n,1,n,a,la,0,g,w,ls,ws(lu1),lws(ll1),.true.)
      call ds_signst(n,r,w,ls)
...
c    *    (e(abs(ls(j))),j=1,n)
   ...
   21 continue
      call ds_optest(peq+1,n-k,r,e,ls,rp,pj)
      if(phase.eq.0)then
...
c  compute +/- Steepest Edge (SE) search direction s in an(.)
        call ds_tfbsub(n,a,la,p,ws(na1),ws(na1),ws(lu1),lws(ll1),
     *    e(p),.true.)
        rp=ds_scpr(0.D0,ws(na1),g,n)
        if(ls(pj).lt.0)rp=-rp
...
c         print 2,'initial rg =',(r(ls(j)),j=n-k+1,n)
          ...
          call ds_store_rg(k,ig,ws(krg1),r,ls(n-k+1))
        endif
c  compute Steepest Descent (SD) search direction s = -Z.rg in an(.)
        call ds_zprod(k,n,a,la,ws(na1),r,w,ls,ws(lu1),lws(ll1))
        rp=ds_scpr(0.D0,ws(na1),g,n)
...
c          print 2,'gg,rp,gg+rp',gg,rp,gg+rp
        ...
        snorm=sqrt(ds_scpr(0.D0,ws(na1),ws(na1),n))
        plus=.true.
      endif
c     print 4,'s (or -s if .not.plus) =',(ws(i),i=na1,na+n)

c  form At.s and denominators
      call ds_form_Ats(n1,lp(1),n,plus,a,la,ws(na1),w,ls,snorm*tol)
...
c       print *,'only one degenerate c/s'
      ...
      if(ninf.gt.0)then
        alpha=amax
        if(plus)then
          call ds_mysaxpy(alpha,ws(na1),x,n)
        else
          call ds_mysaxpy(-alpha,ws(na1),x,n)
        endif
      else
...
c  calculate new x
        if(plus)then
          call ds_saxpyz(alpha,ws(na1),x,ws(nb1),n)
        else
          call ds_saxpyz(-alpha,ws(na1),x,ws(nb1),n)
        endif
        call ds_funct(n,ws(nb1),fp,ws,lws,cws,functions,gradients)
...
c         print 2,'df,z =',df,z
        ...
        f=fp
        call ds_grad(n,ws(nb1),g,ws,lws,cws,functions,gradients)
        ngr=ngr+1
c       print 4,'new g =',(g(i),i=1,n)
        call ds_newg
        gps=ds_scpr(0.D0,g,ws(na1),n)
        if(.not.plus)gps=-gps
...
c       print 4,'new rg =',(w(ls(j)),j=n-k+1,n)
        ...
        call ds_store_rg(k,ig1,ws(krg1),w,ls(n-k+1))
        gpg=0.D0
        gg=0.D0
        do j=n-k+1,n
          i=ls(j)
...
c           print 4,'c =',(ws(kc+i),i=1,ngv)
            call ds_formR(nv,k,ig,maxg,ws(ka1),ws(kb1),ws(kc1),ws(kd1),
     *        ws(ke1),ws(krg1),ws(kr1))
            call ds_formT(nv,maxg,ws(kr1),v,ws(ke1))
c           print 4,'T matrix',(v(i),i=1,nv)
c             if(nv.gt.1)print 5,(ws(ke+i),i=1,nv-1)
            call ds_trid(v,ws(ke1),nv)
...
c  remove contribution to gradient
                if(i.gt.n)then
                  call ds_saipy(sign(1.D0,dble(ls(j))),a,la,i-n,g,n)
                else
                  g(i)=0.D0
                endif
              else
...
c  case: q is a free variable
          ...
          call ds_iexch(ls(qj),ls(n-k))
          ig=0
          if(n_inf.gt.0.and.ninf.eq.0)goto10
          goto15
        endif
        call ds_fbsub(n,n-k,n,a,la,q,w,w,ls,ws(lu1),lws(ll1),.false.)
...
c        if(p.eq.0)print *,'p.eq.0'
        if(p.eq.0)goto98
        call ds_pivot(p,q,n,nmi,a,la,e,ws(lu1),lws(ll1),ifail,npv)
        if(ifail.ge.1)then
...
c          if(phase.gt.0)print *,'phase =',phase
          call ds_iexch(ls(pj),ls(qj))
          call ds_iexch(ls(lp(1)),ls(qj))
          lp(1)=lp(1)-1
          if(ninf.gt.0)goto15
          goto9
        endif
...
c       if(abs(rp-r(p)).gt.1.D-1*gnorm)then
      ...
      snorm=e(p)
c  form At.s and denominators
      call ds_form_Ats(n1,lp(lev),n,plus,a,la,ws(na1),w,ls,snorm*tol)
   86 continue
      if(iprint.ge.3)then
        write(nout,1001)'residual vector and indices',
   ...
   90 continue
      if(qj.eq.0)then
        do j=n1,lp(lev)
          ...
          r(i)=0.D0
        enddo
        call ds_form_Ats(lp(lev)+1,lp(lev-1),n,plus,a,la,ws(na1),
     *    w,ls,snorm*tol)
        lev=lev-1
        f=alp(lev)
...
c1001 format(a/(i3,1x,e14.8,3(i4,1x,e14.8)))
     ...
     * /1111.D-19, 1.D-12, 0.D0, 1.D-8,  2,    3,   2,   500,   0,  0/
      ...
      subroutine ds_optest(jmin,jmax,r,e,ls,rp,pj)
      implicit double precision (a-h,r-z), integer (i-q)
      dimension r(*),e(*),ls(*)
      rp=0.D0
      do 1 j=jmin,jmax
...
c        if(e(i).eq.0.D0)print *,'e(i).eq.0.D0: i =',i
    ...
    1 continue
      ...
      subroutine ds_form_Ats(jmin,jmax,n,plus,a,la,an,w,ls,tol)
      implicit double precision (a-h,r-z), integer (i-q)
...
c  form At.s and denominators
      if(plus)then
        do j=jmin,jmax
          i=abs(ls(j))
          if(i.gt.n)then
            wi=ds_aiscpr(n,a,la,i-n,an,0.D0)
          else
            wi=an(i)
          endif
          if(wi.ne.0.D0)then
      ...
      subroutine ds_zprod(k,n,a,la,an,r,w,ls,aa,ll)
      implicit double precision (a-h,r-z), integer (i-q)
      ...
      do j=n-k+1,n
        w(ls(j))=-r(ls(j))
      enddo
      call ds_tfbsub(n,a,la,0,w,an,aa,ll,ep,.false.)
      return
      end

      subroutine ds_signst(n,r,w,ls)
      implicit double precision (a-h,o-z)
      dimension r(*),w(*),ls(*)
c  transfer with sign change as necessary
      ...
      subroutine ds_stmap(n,nm,kmax,maxg)
...
c  remaining space for use by denseL.f or schurQR.f
      ...
      subroutine ds_warm_start(n,nm,a,la,x,bl,bu,b,ls,aa,ll,an,vstep)
      implicit double precision (a-h,o-z)
     ...
     *  aa(*),ll(*),an(*)
      ...
      do j=1,n
        ...
        if(i.gt.n)then
          if(ls(j).ge.0)then
            b(i)=ds_daiscpr(n,a,la,i-n,x,-bl(i))
          else
            b(i)=ds_daiscpr(n,a,la,i-n,x,-bu(i))
          endif
        endif
      enddo
...
c     write(nout,1000)'r =',(b(abs(ls(j))),j=1,n)
      call ds_tfbsub(n,a,la,0,b,an,aa,ll,ep,.false.)
c     write(nout,1000)'d =',(an(i),i=1,n)
      call ds_linf(n,an,vstep,i)
      if(iprint.ge.1)
...
c1001 format(a/(i4,1x,e11.5,4(i4,1x,e11.5)))
      ...
      subroutine ds_residuals(n,jmin,jmax,a,la,x,bl,bu,r,ls,f,g,ninf)
      implicit double precision (a-h,r-z), integer (i-q)
      dimension a(*),la(*),x(*),bl(*),bu(*),r(*),ls(*),g(*)
      common/epsc/eps,tol,emin
      DOUBLE PRECISION ds_daiscpr,z
      f=0.D0
      ninf=0
      do j=jmin,jmax
        i=abs(ls(j))
        if(i.gt.n)then
          z=ds_daiscpr(n,a,la,i-n,x,0.D0)
        else
          z=dble(x(i))
        endif
        ri=z-dble(bl(i))
      ...
      subroutine ds_store_rg(k,ig,G,r,ls)
      implicit double precision (a-h,o-z)
      dimension G(k,*),r(*),ls(*)
      do j=1,k
        G(j,ig)=r(ls(j))
...
c  This routine is adapted from the EISPAK subroutine imtq11.
   ...
   15 continue
      ...
      subroutine ds_formR(nv,k,ig,maxg,a,b,c,d,e,G,R)
      implicit double precision (a-h,o-z)
   ...
   10 ii=ig-nv
      ...
      do i=1,nv
        ...
        do j=3,nvi1
          jj=jj+1
          if(jj.gt.maxg)jj=1
          d(j)=ds_scpr(0.D0,G(1,jj),G(1,ii),k)
        enddo
        ij=i
        do j=1,i-1
          call ds_mysaxpy(-R(ij),R(ij),d,nvi1)
          ij=ij+maxg-j
        enddo
        if(d(1).le.0.D0)then
...
c     enddo
    ...
    5 format((6E13.5))
      ...
      subroutine ds_formT(n,nmax,R,d,e)
      implicit double precision (a-h,o-z)
      dimension R(*),d(*),e(*)
