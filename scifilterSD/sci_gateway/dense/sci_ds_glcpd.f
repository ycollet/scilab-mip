...
c  CONSTITUTES RECIPIENT'S ACCEPTANCE OF THIS AGREEMENT
      ...
      subroutine sci_ds_glcpd(n,m,k,kmax,maxg,a,la,x,bl,bu,f,fmin,g,r,w,
     * e,ls,alp,lp,mlp,peq,ws,lws,cws,v,nv,rgtol,mode,ifail,mxgr,iprint,
     * nout,my_ds_funct,my_ds_grad)
      implicit double precision (a-h,r-z), integer (i-q)
      external my_ds_funct,my_ds_grad
...
c  collect active equality c/s
   ...
   10 continue
      ...
      n_inf=0
      call my_ds_funct(n,x,f,ws,lws,cws)
      nfn=nfn+1
      if(f.lt.fmin)goto75
      call my_ds_grad(n,x,g,ws,lws,cws)
      ngr=ngr+1
...
c  calculate new x
        ...
        call my_ds_funct(n,ws(nb1),fp,ws,lws,cws)
...
c         print 2,'df,z =',df,z
        ...
        f=fp
        call my_ds_grad(n,ws(nb1),g,ws,lws,cws)
        ngr=ngr+1
