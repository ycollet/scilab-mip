...
c*********************** dense matrix utilities ************************
      ...
      subroutine ds_rsol(n,nn,nmax,R,b)
...
c  and so on. nn indexes the element R(n,n) (where nn=n*(3-n)/2+(n-1)*nmax)
      ...
      do i=n-1,1,-1
        ii=ii-n1+i
        b(i)=-ds_scpr(-b(i),R(ii+1),b(i+1),n-i)/R(ii)
      enddo
      return
      end

      subroutine ds_rtsol(n,nn,nmax,R,b)
      implicit double precision (a-h,o-z)
      dimension R(*),b(*)
...
c  nn is not required on entry but is set on exit
      ...
      do i=2,n
        i1=i-1
        call ds_mysaxpy(-b(i1),R(nn+1),b(i),n-i1)
        nn=nn+n2-i
        b(i)=b(i)/R(nn)
      enddo
      return
      ...
      subroutine ds_Qprod(n,nmax,Q,x,b)
      implicit double precision (a-h,o-z)
      dimension Q(*),x(*),b(*)
c  forms b=M.x where Q is nxn, stored by columns, with stride nmax
      ...
      i1=1
      do i=1,n
        call ds_mysaxpy(x(i),Q(i1),b,n)
        i1=i1+nmax
      enddo
      return
      end

      subroutine ds_Qtprod(n,nmax,Q,x,b)
      implicit double precision (a-h,o-z)
      dimension Q(*),x(*),b(*)
c  forms b=M'.x where Q is nxn, stored by columns, with stride nmax
      i1=1
      do i=1,n
        b(i)=ds_scpr(0.D0,Q(i1),x,n)
        i1=i1+nmax
      enddo
      return
      end

      subroutine ds_brots(n,nmax,k,kk,R,v)
      implicit double precision (a-h,o-z)
      dimension R(*),v(*)
      ipip=kk
      do i=k-1,1,-1
        ...
        ipi=ipip-nmax+i
        ii=ipi-1
        call ds_angle(v(i),v(ip),cos,sin)
        call ds_rot(n-i,R(ipi),R(ipip),cos,sin)
        v(ip)=sin*R(ii)
        R(ii)=cos*R(ii)
        ipip=ii
      enddo
      ...
      subroutine ds_frots(nr,nc,nmax,R,v)
      implicit double precision (a-h,o-z)
      dimension R(*),v(*)
c nr is either nc or nc+1
      ...
      do i=1,nc
        ...
        ipi=ii+1
        ipip=ipi+nmax-i
        call ds_angle(R(ii),v(ip),cos,sin)
        call ds_rot(nr-i,R(ipi),R(ipip),cos,sin)
        ii=ipip
      enddo
      return
      end

      subroutine ds_angle(a,b,cos,sin)
      implicit double precision (a-h,o-z)
      z=sqrt(a**2+b**2)
      if(z.eq.0.D0)then
        cos=1.D0
      ...
      subroutine ds_rot(n,a,b,cos,sin)
      implicit double precision (a-h,o-z)
      dimension a(*),b(*)
      if(sin.eq.0.D0)then
        if(cos.gt.0.D0)then
      ...
      subroutine ds_saxpys(a,x,is,y,n)
      implicit double precision (a-h,o-z)
c  saxpy with stride
      dimension x(*),y(*)
      if(a.eq.0.D0)return
...
c  saxpy with result in x
      ...
      subroutine ds_saxpyz(a,x,y,z,n)
      implicit double precision (a-h,o-z)
c  saxpy with result in z
      ...
      subroutine ds_saxpyi(a,x,y,n)
      implicit double precision (a-h,o-z)
c  saxpy with interchange of x and y
      dimension x(*),y(*)
      if(a.eq.0.D0)then
        do i=1,n
          call ds_rexch(x(i),y(i))
        enddo
      else
        do i=1,n
          z=y(i)
      ...
      function ds_scpr(a,x,y,n)
      implicit double precision (a-h,o-z)
      dimension x(*),y(*)
      ds_scpr=a
      do i=1,n
        ds_scpr=ds_scpr+x(i)*y(i)
      enddo
      return
      end

c     function ds_xlen(a,x,n)
...
c     end
      ...
      function ds_xlen(a,x,n)
      implicit double precision (a-h,o-z)
      dimension x(*)
      ds_xlen=a
      do i=1,n
        ds_xlen=ds_xlen+x(i)**2
      enddo
      ds_xlen=sqrt(ds_xlen)
      return
      end
      
      subroutine ds_linf(n,x,z,iz)
      implicit double precision (a-h,o-z)
      dimension x(*)
      z=0.D0
      do i=1,n
