...
c  Steepest edge coefficients e(i) are also updated in these routines
      ...
      subroutine sp_start_up(n,nm,nmi,a,la,nk,e,ls,aa,ll,mode,ifail)
...
c     write(nout,*)'ls',(ls(ij),ij=1,nk)
      if(mode.eq.3)then
        call sp_re_order(n,nm,a,la(1),la(la(0)),ll,ll(lc1),ll(li1),
...
c         write(nout,*)'failure in re_order (1)'
        ...
        call sp_re_factor(n,nm,a,la,ll,ll(lc1),ll(li1),
     *    ll(lm1),ll(lp1),ll(lq1),ll(lr1),ll(ls1),ll(lt1),aa(np1),
     *    nprof,aa,ifail)
        if(ifail.eq.7)return
        call sp_check_L(n,aa,ll(lp1),ifail)
        if(ifail.eq.1)then
          mode=2
          goto1
        endif
...
c  reset ls from e
        ...
        call sp_EBspace(n,ll(lp1),ll(lq1),ll(ls1),ll,aa(np1),
     *    neb,nprof,ifail)
...
c  set a lower bound on e(i): setting emin=0.D0 will force emin to be recalculated: do this only if mode<3
        emin=1.D0
        do i=1,nmi-n
           emin=max(emin,sp_ailen(n,a,la,i))
        enddo
        emin=1.D0/emin
      endif
      do i=1,n
...
c  shift designated bounds to end and order the resulting rows and columns
        ...
        enddo
        call sp_order(n,nu_,nk,la,ll,ls,ll(li1),ll(lp1),ll(lq1),
     * ll(lr1),aa(np1),nprof,ifail)
        if(ifail.gt.0)return
      endif
      call sp_factor(n,nm,nu_,nk,a,la,e,ls,aa(ns1),aa(nt1),aa(nu1),
...
c         call sp_eptsol(n,a,la,i,a,aa(nq1),aa(nr1),aa(neb1),aa(ny1),
...
c    *  write(nout,*)'error in steepest edge coefficients =',emax
      ...
      subroutine sp_refactor(n,nm,a,la,aa,ll,ifail)
...
c       write(nout,*)'failure in re_order (2)'
      ...
      call sp_re_factor(n,nm,a,la,ll,ll(lc1),ll(li1),ll(lm1),
     ...
     *  nprof,aa,ifail)
      ...
      subroutine sp_pivot(p,q,n,nm,a,la,e,aa,ll,ifail,npv)
      implicit double precision (a-h,r-z), integer (i-q)
...
c     write(nout,*)'pivot: p,q =',p,q
      call sp_updateSE(p,q,n,nm,a,la,e,aa(nq1),aa(nr1),aa(neb1),
...
c       print 3,'li =',(ll(li+i),i=1,nm)
        m1=n-m2
c       call sp_checkperms(n,ll,ll(lc1),ll(li1))
c       mp=-1
c       mq=-1
        call sp_re_order(n,nm,a,la(1),la(la(0)),ll,ll(lc1),ll(li1),
     *    ll(lm1),ll(lp1),ll(lq1),ll(lr1),ll(ls1),ll(lt1),aa(np1),
     ...
     *    neb,nprof,ifail)
        ...
        call sp_updateQR(p,q,n,a,la,aa(nq1),aa(nr1),aa(neb1),aa(nx1),
     ...
     *    ifail)
      ...
      mp=-1
      mq=-1
c     call sp_check_L(n,aa,ll(lp1),ifail)
...
c  check Steepest Edge coefficients
      ...
      do j=1,n
        i=ll(lm+j)
        call sp_eptsol(n,a,la,i,a,aa(nq1),aa(nr1),aa(neb1),aa(ny1),
...
c       if(emax.gt.1.D-2)stop
    ...
    5 format((5E15.7))
      ...
      subroutine sp_fbsub(n,jmin,jmax,a,la,q,b,x,ls,aa,ll,save)
      implicit double precision (a-h,r-z), integer (i-q)
      logical save
    9 format(A/(15I5))
...
c     write(nout,*)'fbsub  q =',q
      ...
      call  sp_aqsol(n,a,la,q,aa(nq1),aa(nr1),aa(neb1),aa(nz1),aa(nt1),
...
c     print *,'x =',(x(i),i=1,18)
      ...
      subroutine sp_ztg(n,k,rg,lv,aa,ll)
      implicit double precision (a-h,r-z), integer (i-q)
...
c     print *,'aa =',(aa(nu+i),i=1,18)
      ...
      subroutine sp_tfbsub(n,a,la,p,b,x,aa,ll,ep,save)
      implicit double precision (a-h,r-z), integer (i-q)
      logical save
...
c     write(nout,*)'tfbsub  p =',p
      call sp_eptsol(n,a,la,p,b,aa(nq1),aa(nr1),aa(neb1),aa(ny1),
...
c******** The following routines are internal to sparseL.f **************
      ...
      subroutine sp_check_L(n,d,p,ifail)
      implicit double precision (a-h,r-z), integer (i-q)
      dimension d(*),p(*)
      common/noutc/nout
...
c     write(nout,*)'m1 =',m1,'   file length =',len,'   total =',len+m1
      ...
      subroutine sp_aqsol(n,a,la,q,Q_,R,EB,z,t,u,x,d,ws,
...
c  scatter a_q into t
        ...
        if(q.le.n)then
          ...
          call sp_iscatter(a,la,q-n,li,t,n)
        endif
      endif
...
c           print 4,'EB =',(EB(j),j=i1,i1+m1-1)
            u(i)=sp_scpr(0.D0,EB(i1),t(m2+1),m1)
...
c  form x=C^{-1}.u
        call sp_Qtprod(mc,mxmc,Q_,u,x)
        mm=mc*(3-mc)/2+(mc-1)*mxmc
        call sp_rsol(mc,mm,mxmc,R,x)
...
c  accumulate t=t+V.x
        do i=1,mc
          ...
          if(lvi.le.n)then
            ...
            call sp_isaipy(x(i),a,la,lvi-n,t,n,lr,li)
          endif
        enddo
      endif
c     print 4,'t in natural order =',(t(li(i)),i=1,n)
c  finally t:=B^{-1}.t-E'.x
      call sp_aqsol0(n,a,la,0,t,u,d,ws,lr,lc,li,pp,qq)
      do i=1,mc
        lei=li(le(i))
        t(lei)=t(lei)-x(i)
      enddo
c     print 4,'t in column order',(t(i),i=1,n)
    ...
    4 format(A/(5E15.7))
      ...
      subroutine sp_aqsol0(n,a,la,q,tn,xn,d,ws,lr,lc,li,pp,qq)
      implicit double precision (a-h,r-z), integer (i-q)
     ...
     *  lr(*),lc(*),li(*),pp(*),qq(*)
      ...
      if(q.gt.0)then
        ...
        if(q.le.n)then
          ...
          call sp_iscatter(a,la,q-n,li,tn,n)
        endif
      endif
c     print *,'tn =',(tn(i),i=1,n)
      do i=n,m2+1,-1
        ...
        if(pri.eq.0)then
          ...
          xn(i)=(sp_scpr(tn(i),ws(qq(ir)+1),tn(i-pri),pri))/d(i)
        endif
        call sp_isaipy(-xn(i),a,la,lc(i)-n,tn,n,lr,li)
      enddo
      do i=m2+1,n
        tn(i)=xn(i)
      enddo
c     print *,'tn =',(tn(i),i=1,n)
      ...
      subroutine sp_eptsol(n,a,la,p,b,Q_,R,EB,y,s,u,x,d,ws,
...
c       print 3,'lv =',(lv(i),i=1,mc)
        do i=1,mc
          ...
          b(le(i))=0.D0
        enddo
        call sp_eptsol0(n,a,la,0,b,s,d,ws,lr,lc,li,pp,qq)
        do i=1,mc
          b(le(i))=x(i)
          if(lv(i).le.n)then
            x(i)=s(li(lv(i)))-b(lv(i))
          else
            x(i)=sp_aiscpri(n,a,la,lv(i)-n,s,-b(lv(i)),lr,li)
          endif
        enddo
      else
...
c  get s=Bk^{-T}.ep
        if(pc.eq.0)then
          call sp_eptsol0(n,a,la,p,a,s,d,ws,lr,lc,li,pp,qq)
...
c  form s=B^{-T}.ep and then y=-V'.s
          do i=1,mc
            if(lv(i).le.n)then
              ...
              x(i)=sp_aiscpri(n,a,la,lv(i)-n,s,0.D0,lr,li)
            endif
            y(i)=-x(i)
          enddo
        else
...
c  form u=C^{-T}.x and accumulate EB'.u into s
        call sp_rtsol(mc,mm,mxmc,R,x)
        call sp_Qprod(mc,mxmc,Q_,x,u)
        i1=1
        do i=1,mc
          call sp_mysaxpy(u(i),EB(i1),s(m2+1),m1)
...
c     print 4,'s =',(s(i),i=1,n)
    ...
    4 format(A/(5E15.7))
      ...
      subroutine sp_eptsol0(n,a,la,p,b,sn,d,ws,lr,lc,li,pp,qq)
      implicit double precision (a-h,r-z), integer (i-q)
     ...
     *  lr(*),lc(*),li(*),pp(*),qq(*)
      ...
      if(p.eq.0)then
        ...
        do i=m2+1,n
          j=lc(i)
          sn(i)=-sp_aiscpri(n,a,la,j-n,sn,-b(j),lr,li)/d(i)
          ir=lr(i)
          pri=pp(ir)
          if(pri.gt.0)call sp_mysaxpy(sn(i),ws(qq(ir)+1),sn(i-pri),pri)
        enddo
      else
        do i=1,n
          sn(i)=0.D0
        ...
        if(p.le.n)then
          ...
          sn(pr)=1.D0
          do i=m2+1,n
            sn(i)=-sp_aiscpri(n,a,la,lc(i)-n,sn,0.D0,lr,li)/d(i)
            ir=lr(i)
            pri=pp(ir)
            if(pri.gt.0)call sp_mysaxpy(sn(i),ws(qq(ir)+1),sn(i-pri),
     * pri)
          enddo
        else
          if(pr.le.m2)goto1
          do i=m2+1,n
            bi=0.D0
            if(i.eq.pr)bi=-1.D0
            sn(i)=-sp_aiscpri(n,a,la,lc(i)-n,sn,bi,lr,li)/d(i)
            ir=lr(i)
            pri=pp(ir)
            if(pri.gt.0)call sp_mysaxpy(sn(i),ws(qq(ir)+1),sn(i-pri),
     * pri)
...
c     print *,'sn =',(sn(i),i=1,n)
    ...
    1 continue
      ...
      subroutine sp_order(n,nu,nc,la,lr,ls,li,p,q,r,ws,mxws,ifail)
...
c     write(nout,*)'lower profile =',(li(abs(ls(i))),i=nu+1,nc)
      ...
      subroutine sp_factor(n,nm,nu,nc,a,la,e,ls,sn,tn,un,xn,lr,lc,li,
...
c       write(nout,*)'coli =',coli
        ...
        do i=1,n
          tn(i)=0.D0
        enddo
        call sp_iscatter(a,la,coli-n,li,tn,n)
        do i=m1,1,-1
          rowi=lr(i)
          pri=p(rowi)
          if(pri.eq.0)then
            ...
            xn(i)=(sp_scpr(tn(i),ws(q(rowi)+1),tn(i-pri),pri))/d(i)
          endif
          call sp_isaipy(-xn(i),a,la,lc(i)-n,tn,n,lr,li)
        enddo
        do i=1,m1
...
c  threshold pivot selection
        call sp_linf(tu-m1,tn(m1p),z,iz)
        if(z.le.tol)then
          li(coli)=0
          goto2
        endif
...
c       write(nout,*)'zz,z,iz,m1,qri',zz,z,iz,m1,qri
        if(iz.gt.m1p)then
          call sp_rexch(tn(m1p),tn(iz))
          call sp_iexch(lr(m1p),lr(iz))
          li(lr(m1p))=m1p
          li(lr(iz))=iz
        endif
        rowp=lr(m1p)
c  reset q values
        ...
        do i=1,m1
          ...
          rowi=lr(i)
          pri=p(rowi)
        if(pri.gt.0)call sp_mysaxpy(sn(i),ws(q(rowi)+1),sn(i-pri),pri)
        enddo
...
c  update steepest edge coefficients
        ...
        do i=m1,1,-1
          ...
          if(pri.eq.0)then
            ...
            xn(i)=(sp_scpr(un(i),ws(q(rowi)+1),un(i-pri),pri))/d(i)
          endif
          call sp_isaipy(-xn(i),a,la,lc(i)-n,un,n,lr,li)
        enddo
        do i=1,m1
          un(i)=xn(i)
        enddo
c       write(nout,*)'un =',(un(i),i=1,n)
    ...
    1   continue
        pri=m1p-j
        if(pri.gt.0)then
          call sp_newslot(rowp,pri,lastr,irow,p,q,r,s,ws,mxws,i,ifail)
          if(ifail.gt.0)return
          p(rowp)=pri
          i=q(rowp)
          do j=j,m1
...
c     call sp_checkout(n,a,la,lr,lc,li,p,q,r,s,ws,mxws,d)
    4 format(A/(5E15.7))
    9 format(A/(15I5))
      return
      end

      subroutine sp_re_order(n,nm,a,la,point,lr,lc,li,mao,p,q,r,s,
     *  t,ws,mxws,ifail)
...
c     write(nout,*)'lower profile =',(t(j),j=nu+1,n)
      ...
      subroutine sp_re_factor(n,nm,a,la,lr,lc,li,mao,p,q,r,s,
     *  t,ws,mxws,d,ifail)
      implicit double precision (a-h,u-z), integer (i-t)
...
c         write(nout,*)'inode,t(inode)',inode,t(inode)
          do j=inode,t(inode)
            ...
            prj=p(rowj)
            if(prj.gt.0)then
              d(j)=sp_aiscpri2(n,a,la,rowj,nodec,ws(q(rowj)+1),1.D0,im,
     *          prj,li)
            else
              d(j)=sp_aij(rowj,nodec,a,la)
            endif
            z=max(z,abs(d(j)))
          enddo
...
c       write(nout,*)'zz,z,iz,pri',zz,z,iz,pri
          if(iz.gt.inode)then
c  pivot interchange
            call sp_rexch(d(inode),d(iz))
            call sp_iexch(lr(inode),lr(iz))
            li(lr(iz))=iz
            li(lr(inode))=inode
          endif
...
c  special case of a zero multiplier
              ...
              len_=prj+1
              if(len_.gt.len)then
              call sp_newslot(rowj,len_,lastr,irow,p,q,r,s,ws,mxws,qrj,
     *            ifail)
                if(ifail.gt.0)return
                qrj_=q(rowj)
                do k=1,prj
...
c  create a new slot and use saxpyz ...
     ...
     *          ifail)
              ...
              if(len.ge.0)then
                ...
                enddo
                len=len+1
                call sp_saxpyz(z,ws(qri+1),ws(qrj+len),ws(qrj_+len),
     *            len_-len)
              else
                len=-len
                do k=1,len
                ...
                enddo
                len=len+1
                call sp_saxpyz(z,ws(qri+len),ws(qrj+1),ws(qrj_+len),
     *            len_-len)
              endif
              ws(qrj_+len_)=z
            else
c  ... else saxpy in-situ
              if(pri.gt.0)
     *          call sp_mysaxpy(z,ws(qri+1),ws(qrj+prj-pri+1),pri)
              ws(qrj+len_)=z
            endif
            p(rowj)=len_
c           do rj=1,n
...
c         enddo
        ...
        pri=p(noder)
        if(pri.gt.0)then
         d(inode)=sp_aiscpri2(n,a,la,noder,lc(inode)-n,ws(q(noder)+1),
     *     1.D0,inode-1,pri,li)
        else
          d(inode)=sp_aij(noder,lc(inode)-n,a,la)
        endif
        if(d(inode).eq.0.D0)d(inode)=eps
      endif
      i=mao(i)+1
...
c     write(nout,*)'lc =',(lc(j),j=nu+1,n)
      ...
      function sp_aiscpri2(n,a,la,rowi,coli,ws,di,im,pri,li)
      implicit double precision (a-h,o-z)
      ...
      do j=la(jp),la(jp+1)-1
        ...
        if(rowj.eq.rowi)then
          sp_aiscpri2 = sp_aiscpri2+di*a(j)
        else
          ir=li(rowj)-im
          if(ir.gt.0)goto1
          ir=ir+pri
          if(ir.gt.0) sp_aiscpri2 = sp_aiscpri2+ws(ir)*a(j)
        endif
    1   continue
      enddo
      return
      ...
      subroutine sp_updateSE(p,q,n,nm,a,la,e,Q_,R,EB,y,z,s,t,u,x,
...
c     print 3,'lm =',(lm(i),i=1,n)
      if(p.ne.mp)then
        call sp_eptsol(n,a,la,p,a,Q_,R,EB,y,s,u,x,d,ws,
     *    lr,lc,li,lv,le,pp,qq,ep)
...
c       print 4,'sk ordered by lr',(s(i),i=1,n)
      ...
      if(q.ne.mq)then
        call sp_aqsol(n,a,la,q,Q_,R,EB,z,t,u,x,d,ws,
     *    lr,lc,li,lv,le,pp,qq)
...
c  update steepest edge coefficients
      ...
      do i=1,n
        u(i)=eq*s(i)
      enddo
      call sp_aqsol(n,a,la,0,Q_,R,EB,x,u,s,x,d,ws,
     *  lr,lc,li,lv,le,pp,qq)
...
c     print 4,'e =',(e(i),i=1,nm)
      ...
      subroutine sp_updateQR(p,q,n,a,la,Q_,R,EB,x,y,z,
...
c           print 2,'z =',(z(i),i=1,mc)
            ...
            mmx1=mmx+1
            do i=1,mc
              call sp_angle(R(ii),y(i),cos,sin)
              call sp_rot(mc-i,R(ii+1),y(i+1),cos,sin)
              call sp_rot(mc1,Q_(ic),Q_(mmx1),cos,sin)
              ii=ii+mxmc-i+1
              ic=ic+mxmc
            enddo
            z(mc1)=y(mc1)
...
c         print 3,'lm =',(lm(i),i=1,n)
          ...
          if(mc.gt.0)then
            icp=mxmc*mc+mc1
            ic=icp
            call sp_rexch(Q_(icp),Q_(icp-j))
            ii=mc1*(3-mc1)/2+(mc1-1)*mxmc
            z(mc1)=R(ii)
            do i=mc,1,-1
              ic=ic-mxmc
              call sp_rexch(Q_(ic),Q_(ic-j))
              call sp_angle(Q_(icp),Q_(ic),cos,sin)
              call sp_rot(mc,Q_(icp-mc),Q_(ic-mc),cos,sin)
              ii=ii-mxmc+i-1
              z(i)=sin*R(ii)
              R(ii)=-cos*R(ii)
              call sp_rot(mc1-i,z(i+1),R(ii+1),cos,sin)
            enddo
            ic=1
            icp=icp-mc
            do i=1,mc
              call sp_angle(R(ii),y(i),cos,sin)
              call sp_rot(mc1-i,R(ii+1),y(i+1),cos,sin)
              call sp_rot(mc1,Q_(ic),Q_(icp),cos,sin)
              ii=ii+mxmc-i+1
              ic=ic+mxmc
            enddo
            R(ii)=y(mc1)
...
c  first remove column pc from V
        call sp_ishift(lv(pc),mc-pc,1)
        ic=pc
        do i=1,pc-1
          call sp_r_shift(R(ic),mc-pc,1)
          ic=ic+mxmc-i
        enddo
        mc1=mc
        mc=mc-1
        ...
        ic=(pc-1)*mxmc+1
        do i=pc,mc
          call sp_angle(R(ii),R(iip),cos,sin)
          call sp_rot(mc-i,R(ii+1),R(iip+1),cos,sin)
          call sp_r_shift(R(ii-1),mc1-i,1)
          ii=iip+1
          iip=ii+mxmc-i-1
          icp=ic+mxmc
          call sp_rot(mc1,Q_(ic),Q_(icp),cos,sin)
...
c         print *,'contract',p,q
          ...
          do i=mc,1,-1
            ...
            y(i)=sin*R(ii)
            R(ii)=-cos*R(ii)
            call sp_rot(mc-i,y(i+1),R(ii+1),cos,sin)
            ii=ii-mxmc+i-2
          enddo
          le(qr)=le(mc1)
          j=m1*(mc1-qr)
...
c     print 4,'EB in check =',(EB(i),i=1,m1mc)
      do j=1,mc
        ...
        if(lv(j).le.n)then
          ...
          call sp_iscatter(a,la,lv(j)-n,li,x,n)
        endif
        do i=1,mc
          z(i)=0.D0
        enddo
        i1=j
        do i=1,j
          call sp_mysaxpy(R(i1),Q_((i-1)*mxmc+1),z,mc)
          i1=i1+mxmc-i
        enddo
...
c       print 4,'x =',(x(i),i=1,n)
        emax=0.D0
        do i=1,mc
          cij=-sp_scpr(0.D0,EB((i-1)*m1+1),x(m2+1),m1)
...
c         endif
      ...
      subroutine sp_newslot(row,len,lastr,irow,p,q,r,s,ws,mxws,qr_,
     *  ifail)
      implicit double precision (a-h,u-z), integer (i-t)
      parameter (igap=10)
...
c  compress ws
      ...
      qrow=0
    3 continue
      call sp_r_shift(ws(qrow+1),p(thisr),q(thisr)-qrow)
      q(thisr)=qrow
      qrow=qrow+p(thisr)
...
c  insert after row thisr and take out old row
        call sp_erase(row,lastr,irow,r,s)
      endif
      lastr=row
      r(row)=thisr
      ...
      subroutine sp_erase(row,lastr,irow,r,s)
...
c     write(nout,*)'erase: row,irow,lastr =',row,irow,lastr
      ...
      subroutine sp_trim_(rowi,pri,qri,q,ws)
c  trim leading zeros off slot for row i
    ...
    1 continue
      ...
      subroutine sp_EBspace(n,p,q,s,lr,ws,neb,nprof,ifail)
      implicit double precision (a-h,u-z), integer (i-t)
...
c  compress the file
    ...
    9 format(A/(15I5))
      ...
      subroutine sp_checkperms(n,lr,lc,li)
      implicit double precision (a-h,r-z), integer (i-q)
      dimension lr(*),lc(*),li(*)
      common/factorc/m1,m2,mp,mq,lastr,irow
        do i=1,n
      ...
      subroutine sp_checkout(n,a,la,lr,lc,li,p,q,r,s,ws,mxws,d)
      implicit double precision (a-h,r-z), integer (i-q)
...
c  form L.a_q
        ...
        elseif(prj.eq.0)then
          e=abs(sp_aij(rowj,nodec-n,a,la)-d(inode))
        else
          e=abs(d(inode)-sp_aiscpri2(n,a,la,rowj,nodec-n,ws(q(rowj)+1),
     *      1.D0,inode-1,prj,li))
        endif
...
c    *    '  inode,nodec,rowj =',inode,nodec,rowj
        ...
        do j=inode+1,n
          ...
          prj=p(rowj)
          if(prj.gt.0)then
            e=abs(sp_aiscpri2(n,a,la,rowj,nodec-n,ws(q(rowj)+1),1.D0,
     *         j-1,prj,li))
          else
            e=abs(sp_aij(rowj,nodec-n,a,la))
          endif
c         if(e.gt.tol)write(nout,*)'error =',e,
