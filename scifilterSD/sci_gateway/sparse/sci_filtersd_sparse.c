...
void C2F(sparse_functions)(int * n, int * m, double * x, double * f, double * c, double * user, int * iuser);
void C2F(sparse_gradients)(int * n, int * m, double * x, double * a, double * user, int * iuser);
...
int sci_filtersd_sparse(char * fname)
{
  ...
  // loaded via "link".
  func_obj = (fobj_filtersd_ptr)GetFunctionPtr("filtersd_fobj", FOBJ_IN, FTab_filtersd_function, (voidf)C2F(sparse_functions), &sci_obj, &lhs_obj, &rhs_obj);
  ...
  if (func_obj!=(voidf)C2F(sparse_functions))
    {
  ...
  // Here, we get either a Scilab script function or a string which gives the name of the C/C++/Fortran function
  // loaded via "link".
  func_grad = (grad_filtersd_ptr)GetFunctionPtr("filtersd_grad", GRAD_IN, FTab_filtersd_function, (voidf)C2F(sparse_gradients), &sci_obj, &lhs_obj, &rhs_obj);
  ...
  if (func_grad!=(voidf)C2F(sparse_gradients))
    {
  ...
  if (!is_external_func)
      ...
      if (Err>0) 
	{
      ...
      _SciErr = createMatrixOfDouble(pvApiCtx, param_grad.stack_pos+1, 0, 0, NULL); FILTERSD_ERROR;
      _SciErr = createMatrixOfDouble(pvApiCtx, param_grad.stack_pos+2, 0, 0, NULL); FILTERSD_ERROR;
      _SciErr = createMatrixOfDouble(pvApiCtx, param_grad.stack_pos+3, 0, 0, NULL); FILTERSD_ERROR;
      ...
      if (Err>0) 
	{
      ...
      _SciErr = getVarAddressFromPosition(pvApiCtx, param_grad.stack_pos+1, &grad_c_index_addr); FILTERSD_ERROR;
      _SciErr = getMatrixOfDouble(pvApiCtx, grad_c_index_addr, &m_grad_c_index, &n_grad_c_index, &grad_c_index); FILTERSD_ERROR;
      
      _SciErr = getVarAddressFromPosition(pvApiCtx, param_grad.stack_pos+2, &grad_c_start_addr); FILTERSD_ERROR;
      _SciErr = getMatrixOfDouble(pvApiCtx, grad_c_start_addr, &m_grad_c_start, &n_grad_c_start, &grad_c_start); FILTERSD_ERROR;
      
      _SciErr = getVarAddressFromPosition(pvApiCtx, param_grad.stack_pos+3, &grad_c_val_addr); FILTERSD_ERROR;
      _SciErr = getMatrixOfDouble(pvApiCtx, grad_c_val_addr, &m_grad_c_val, &n_grad_c_val, &grad_c_val); FILTERSD_ERROR;

      FREE(lws);
      lws = (int *)MALLOC((3+m_grad_c_index*n_grad_c_index+m_grad_c_start*n_grad_c_start + 1000)*sizeof(int));
      
      k = 0;
      lws[0] = 1 + n + m_grad_c_index*n_grad_c_index;
      
      // Store the objective function gradient
      lws[1+1] = 1;
      for(i=0;i<n;i++)
	{
	  lws[k+1] = i+1;
      ...
      // Store the constraint functions gradient
      for(i=0;i<m_grad_c_index*n_grad_c_index;i++)
	{
	  lws[k+1] = grad_c_index[i];
	  ws[k]    = grad_c_val[i];
	  k++;
	}
      ...
      lws[k+1] = 1;
      k++;
      for(i=0;i<m_grad_c_start*n_grad_c_start;i++)
	{
	  lws[k+1] = grad_c_start[i] + n;
	  k++;
	}
      lws[k+1] =  1 + n + m_grad_c_index*n_grad_c_index;
      ...
      maxa  = 1 + n + m_grad_c_index*n_grad_c_index;
      maxla = maxa + m + 1;

#ifdef DEBUG
      for(i=0;i<3+n+m_grad_c_index*n_grad_c_index+m_grad_c_start*n_grad_c_start;i++)
	{
	  printf("DEBUG: sparse: lws[%d] = %d\n", i, lws[i]);
	}
      for(i=0;i<m_grad_c_index*n_grad_c_index+n;i++)
	{
	  printf("DEBUG: sparse: ws[%d] = %f\n", i, ws[i]);
	}
#endif
      Nbvars = nbvars_old;
    }
  ...
  if (!is_external_func)
    {
      C2F(sp_filtersd)(&n,&m,x_filtersd,lambda_out,&f_tmp_out,&fmin,cstype,x_lower_filtersd,x_upper_filtersd,
		       ws,lws,v,&nv,&maxa,&maxla,&maxu,&maxiu,&kmax,&maxg,&rho,&htol,&rgtol,&maxit,&iprint,&nout,&ifail,
		       C2F(sparse_functions),C2F(sparse_gradients));
    }
  else
    {
      C2F(sp_filtersd)(&n,&m,x_filtersd,lambda_out,&f_tmp_out,&fmin,cstype,x_lower_filtersd,x_upper_filtersd,
...
void C2F(sparse_functions)(int * n, int * m, double * x, double * f, double * c, double * user, int * iuser)
{
  ...
  if (param_fobj.fobj_type==0)
  ...
  else
    {
      ...
      Scierror(999,"filtersd_sparse: call to C functions not implemented yet\n");
      return;
    }
...
void C2F(sparse_gradients)(int * n, int * m, double * x, double * a, double * user, int * iuser)
{
  ...
  int n_grad_c = 0, m_grad_c = 0, * grad_c_addr = NULL; double * grad_c = NULL;
  int n_x = 0;
  int n_c = 0;
  ...
  if (param_grad.fobj_type==0)
    {
      ...
      Nbvars = stack_pos + MAX(rhs_obj,lhs_obj) + 4;
      ...
      _SciErr = createMatrixOfDouble(pvApiCtx, stack_pos+0, n_x, 1, x); FILTERSD_ERROR_NORETURN;
      _SciErr = createMatrixOfDouble(pvApiCtx, stack_pos+1, 0, 0, NULL); FILTERSD_ERROR_NORETURN;
      _SciErr = createMatrixOfDouble(pvApiCtx, stack_pos+2, 0, 0, NULL); FILTERSD_ERROR_NORETURN;
      _SciErr = createMatrixOfDouble(pvApiCtx, stack_pos+3, 0, 0, NULL); FILTERSD_ERROR_NORETURN;
      ...
      if (Err>0) 
	{
      ...
      _SciErr = getVarAddressFromPosition(pvApiCtx, stack_pos+0, &grad_f_addr); FILTERSD_ERROR_NORETURN;
      _SciErr = getMatrixOfDouble(pvApiCtx, grad_f_addr, &m_grad_f, &n_grad_f, &grad_f); FILTERSD_ERROR_NORETURN;
#ifdef DEBUG
      printf("DEBUG: m_grad_f = %d, n_grad_f = %d\n", m_grad_f, n_grad_f);
#endif

      _SciErr = getVarAddressFromPosition(pvApiCtx, stack_pos+1, &grad_c_addr); FILTERSD_ERROR_NORETURN;
      _SciErr = getMatrixOfDouble(pvApiCtx, grad_c_addr, &m_grad_c, &n_grad_c, &grad_c); FILTERSD_ERROR_NORETURN;
#ifdef DEBUG
      printf("DEBUG: m_grad_c = %d, n_grad_c = %d\n", m_grad_c, n_grad_c);
#endif

      _SciErr = getVarAddressFromPosition(pvApiCtx, stack_pos+2, &grad_c_addr); FILTERSD_ERROR_NORETURN;
      _SciErr = getMatrixOfDouble(pvApiCtx, grad_c_addr, &m_grad_c, &n_grad_c, &grad_c); FILTERSD_ERROR_NORETURN;
#ifdef DEBUG
      printf("DEBUG: m_grad_c = %d, n_grad_c = %d\n", m_grad_c, n_grad_c);
#endif

      _SciErr = getVarAddressFromPosition(pvApiCtx, stack_pos+3, &grad_c_addr); FILTERSD_ERROR_NORETURN;
      _SciErr = getMatrixOfDouble(pvApiCtx, grad_c_addr, &m_grad_c, &n_grad_c, &grad_c); FILTERSD_ERROR_NORETURN;
#ifdef DEBUG
      printf("DEBUG: m_grad_c = %d, n_grad_c = %d\n", m_grad_c, n_grad_c);
#endif
      ...
      for(i=0;i<n_grad_f*m_grad_f;i++)
	{
	  a[k] = grad_f[i];
	  k++;
	}

      for(i=0;i<n_grad_c*m_grad_c;i++)
	{
	  a[k] = grad_c[i];
	  k++;
	}
      ...
      for(i=0;i<n_grad_f*m_grad_f+n_grad_c*m_grad_c;i++)
	{
	  printf("DEBUG: sparse: a[%d] = %f\n", i, a[i]);
	}
#endif
