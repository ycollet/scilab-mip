      ...
      subroutine sp_filterSD(n,m,x,al,f,fmin,cstype,bl,bu,ws,lws,v,nv,
...
c  note x and al are just used as workspace: the true values are those in ws
      ...
      call sp_filter_SD(n,f,fmin,cstype,bl,bu,ws,lws,v,nv,
     *  maxa,kmax,maxg,
...
c  scatter ws(nx.. and ws(nal.. and bound multipliers into x and al
    ...
    1 format(A,15I5)
      ...
      subroutine sp_filter_SD(n,f,fmin,cstype,bl,bu,ws,lws,v,nv,
     *  maxa,kmax,maxg,
...
c     if(itn.eq.164)iii=1
      call sp_glcpd(n,m,k,kmax,maxg,ws(last1),lws(nla1),d,dl,du,phi,
     *  -ainfty,g,r,w,e,ls,alp,lp,mlp,ipeq,ws,lws,cstype,v,nv,rgtol,
     *  mode,ifail,infty,iii,0,functions,gradients)
...
c     print *,'LP is infeasible: solve l1 subproblem',itn
      iii=0
c     if(itn.eq.9)iii=2
      call sp_l1sold(n,m,k,kmax,maxg,ws(last1),lws(nla1),d,dl,du,phi,
...
c     print 3,'ls =',(ls(i),i=1,nm)
      ...
      if(abs(iph).eq.2)then
        call sp_addfil(h,f,filh,filf,1,nfil,mxf,ifail)
...
c       call sp_addfil(hJt,hJ,filh,filf,nfil1,nfil,mxf,ifail)
c       if(ifail.gt.0)return
        call sp_testfil(hJt,hJ,filh,filf,nfil1,nfil,ifail)
        if(ifail.eq.1)then
...
c     print 4,'al =',(ws(i),i=nal1,nal+m)
      do i=1,m
        call sp_saipy(ws(nal+i),ws(last1),lws(nla1),i,ws(naal1),n)
      enddo
   20 continue
...
c  filter test for LCP solution
      call sp_testfil(hxdJt,hxdJ,filh,filf,nfil1,nfil,ifail)
      if(ifail.eq.0)call sp_testfil(hxdJt,hxdJ,hJt,hJ,1,1,ifail)
...
c       if(itn.eq.68)iii=1
        call sp_glcpd(n,m,k,kmax,maxg,ws(next1),lws(nla1),d,dl,du,phi,
...
c  accept LCP (iph=1) or projection (iph=-1) solution
      if(dq.lt.tol)then
        call sp_addfil(hJt,hJ,filh,filf,nfil1,nfil,mxf,ifail)
        if(ifail.gt.0)return
      endif
      do i=1,n
        ws(nx+i)=ws(nxd+i)
...
c     print *,'solve phase 2 LCP subproblem',itn
      iii=0
c     if(itn.eq.164)iii=1
      call sp_glcpd(n,m,k,kmax,maxg,ws(last1),lws(nla1),d,dl,du,phi,fmin,
...
c  filter test for LCP solution
      ...
      df=f-fxd
c     print 2,'dq,df',dq,df
      call sp_testfil(hxd,fxd,filh,filf,1,nfil,ifail)
      if(ifail.eq.0)call sp_testfil(hxd,fxd,h,f,1,1,ifail)
...
c     iph=2
   ...
   99 continue
     ...
     *    / 1.D20,  1.D4, 50,  50, 1000000/
      ...
      subroutine sp_testfil(h,f,filh,filf,nfil1,nfil,ifail)
      implicit double precision (a-h,o-z)
      dimension filh(*),filf(*)
...
c       if(hd.gt.filh(i).and.fp.gt.filf(i))then
      ...
      subroutine sp_addfil(h,f,filh,filf,nfil1,nfil,mxf,ifail)
      implicit double precision (a-h,o-z)
      dimension filh(*),filf(*)
      do i=nfil,nfil1,-1
      ...
      subroutine sp_funct(n,d,phi,ws,lws,cstype,functions,gradients)
      implicit double precision (a-h,o-z)
      dimension d(*),ws(*),lws(*)
      character cstype(*)
      external functions,gradients
...
c  projection subproblem
        phi=5.D-1*sp_scpr(0.D0,d,d,n)
        return
      elseif(iph.eq.0)then
c  LP subproblem
        phi=sp_aiscpr(n,ws(last1),lws(nla1),0,d,0.D0)
        return
      endif
      do i=1,n
        ws(nxd+i)=ws(nx+i)+d(i)
...
c     print *,'$cstype =',(cstype(i),i=1,m)
      if(iph.eq.2)then
        phi=sp_scpr(-sp_scpr(-fxd-alc,ws(nal1),ws(ncxd1),m),ws(naal1),
     *     d,n)
      else
        phi=sp_scpr(-sp_scpr(-alc,ws(nal1),ws(ncxd1),m),ws(naal1),d,n)
        do i=1,m
          if(cstype(i).eq.'A')then
            phi=phi-ws(ncxd+i)
          elseif(cstype(i).eq.'Z')then
...
c     print 2,'$phi =',phi
      ...
      subroutine sp_grad(n,d,g,ws,lws,cstype,functions,gradients)
      implicit double precision (a-h,o-z)
     ...
     *  nal,nal1,naal,naal1,nxd,nxd1,ncx,ncx1,ncxd,ncxd1,nla1
      ...
      elseif(iph.eq.0)then
        do i=1,n
          g(i)=0.D0
        enddo
        call sp_saipy(1.D0,ws(last1),lws(nla1),0,g,n)
        return
      endif
c     print 4,'£x+d =',(ws(nxd+i),i=1,n)
      call gradients(n,m,ws(nxd1),ws(next1),ws,lws)
...
c     print 4,'£Ak.al =',(g(i),i=1,n)
      do i=1,m
        call sp_saipy(-ws(nal+i),ws(next1),lws(nla1),i,g,n)
      enddo
      if(iph.eq.2)then
        call sp_saipy(1.D0,ws(next1),lws(nla1),0,g,n)
      else
        do i=1,m
          if(cstype(i).eq.'A')then
            call sp_saipy(-1.D0,ws(next1),lws(nla1),i,g,n)
          elseif(cstype(i).eq.'Z')then
            call sp_saipy(1.D0,ws(next1),lws(nla1),i,g,n)
          endif
        enddo
