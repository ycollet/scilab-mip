c  Copyright (C) 2010 Roger Fletcher
      ...
      subroutine sp_l1sold(n,m,k,kmax,maxg,a,la,x,bl,bu,f,g,r,w,e,ls,
...
c     call sp_check1(n,lp(1),nm,k,kmax,g,a,la,x,bl,bu,r,ls,lp,ws(nb1),f,
c    *  ws,lws,cws,1,p,rp)

   21 continue
c  l1 optimality test
      call sp_optest1(peq,k,n,bl,bu,r,e,ls,rp,pj)
...
c     print 2,'slope for r(169) =',sp_aiscpr(n,a,la,169-n,ws(na1),0.D0)
c     print 2,'slope for r(217) =',sp_aiscpr(n,a,la,217-n,ws(na1),0.D0)
c     print *,'plus =',plus
...
c  pivot interchange
        ...
        call sp_pivot(p,q,n,nm,a,la,e,ws(lu1),lws(ll1),ifail,npv)
        if(ifail.ge.1)then
        ...
        elseif(r(p).gt.0.D0)then
          call sp_saipy(-sign(1.D0,dble(ls(pj))),a,la,p-n,g,n)
          call sp_newg
          r(p)=-alpha
        else
          rpu=bu(p)-bl(p)-alpha
...
c1001 format(a/(i3,1x,e14.8,3(i4,1x,e14.8)))
      ...
      subroutine sp_optest1(peq,k,n,bl,bu,r,e,ls,rp,pj)
      implicit double precision (a-h,r-z), integer (i-q)
...
c  additional test for free variables
      ...
      subroutine sp_check1(n,nm,nmi,k,kmax,g,a,la,x,bl,bu,r,ls,lp,an,f,
