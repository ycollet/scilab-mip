...
c  CONSTITUTES RECIPIENT'S ACCEPTANCE OF THIS AGREEMENT
      ...
      subroutine sp_glcpd(n,m,k,kmax,maxg,a,la,x,bl,bu,f,fmin,g,r,w,e,
     * ls,alp,lp,mlp,peq,ws,lws,cws,v,nv,rgtol,mode,ifail,mxgr,iprint,
     * nout,functions,gradients)
      implicit double precision (a-h,r-z), integer (i-q)
      external functions,gradients
...
C  mode  mode of operation (larger numbers imply extra information):
...
C        A local copy (mode) of mode is made and may be changed by glcpd
...
c  set up factors of basis matrix and permutation vectors
        ifail=mode
        call sp_start_up(n,nm,nmi,a,la,nk,e,ls,ws(lu1),lws(ll1),mode,
     * ifail)
...
c  refinement step loop
      ...
      enddo
      if(mode.gt.0)then
        call sp_warm_start(n,nm,a,la,x,bl,bu,r,ls,ws(lu1),
...
c  move inactive general c/s to the end
      do j=nm,n1,-1
        i=abs(ls(j))
        if(i.gt.n)then
          call sp_iexch(ls(j),ls(lp(1)))
          lp(1)=lp(1)-1
        endif
      enddo
      call sp_residuals(n,n1,lp(1),a,la,x,bl,bu,r,ls,f,g,ninf)
      if(ninf.gt.0)then
        gnorm=sqrt(dble(ninf))
        gtol=sgnf*gnorm
        rgtol=max(rgtol,gtol)
        goto15
      endif
    9 continue
c  enter phase 1
...
c  collect active equality c/s
      do j=1,n-k
        ...
        if(bu(i).eq.bl(i))then
          peq=peq+1
          call sp_iexch(ls(j),ls(peq))
        endif
      enddo
      call sp_residuals(n,lp(1)+1,nm,a,la,x,bl,bu,r,ls,f,g,ninf)
      lp(1)=nm
      if(ninf.gt.0)then
        gnorm=sqrt(sp_scpr(0.D0,g,g,n))
        gtol=sgnf*gnorm
        rgtol=max(rgtol,gtol)
        goto15
      endif
   10 continue
      ...
      n_inf=0
      call sp_funct(n,x,f,ws,lws,cws,functions,gradients)
      nfn=nfn+1
      if(f.lt.fmin)goto75
      call sp_grad(n,x,g,ws,lws,cws,functions,gradients)
      ngr=ngr+1
...
c     print 4,'gradient =',(g(i),i=1,n)
      do i=1,nm
        w(i)=0.D0
      enddo
      call sp_fbsub(n,1,n,a,la,0,g,w,ls,ws(lu1),lws(ll1),.true.)
      call sp_signst(n,r,w,ls)
...
c     call sp_check(n,lp(1),nmi,kmax,g,a,la,x,bl,bu,r,ls,ws(nb1),f,
c    *  ws,lws,cws,ninf,peq,k,1,p,rp)

   21 continue
      call sp_optest(peq+1,n-k,r,e,ls,rp,pj)
      if(phase.eq.0)then
...
c  compute +/- Steepest Edge (SE) search direction s in an(.)
        call sp_tfbsub(n,a,la,p,ws(na1),ws(na1),ws(lu1),lws(ll1),
     *    e(p),.true.)
        rp=sp_scpr(0.D0,ws(na1),g,n)
        if(ls(pj).lt.0)rp=-rp
...
c         print 2,'initial rg =',(r(ls(j)),j=n-k+1,n)
          ...
          call sp_store_rg(k,ig,ws(krg1),r,ls(n-k+1))
        endif
c  compute Steepest Descent (SD) search direction s = -Z.rg in an(.)
        call sp_zprod(k,n,a,la,ws(na1),r,w,ls,ws(lu1),lws(ll1))
        rp=sp_scpr(0.D0,ws(na1),g,n)
        if(abs(gg+rp).gt.5.D-1*max(gg,abs(rp)))then
c       if(abs(gg+rp).gt.1.D-2*max(gg,abs(rp)))then
c          print 2,'gg,rp,gg+rp',gg,rp,gg+rp
          goto98
        endif
        snorm=sqrt(sp_scpr(0.D0,ws(na1),ws(na1),n))
        plus=.true.
      endif
c     print 4,'s (or -s if .not.plus) =',(ws(i),i=na1,na+n)

c  form At.s and denominators
      call sp_form_Ats(n1,lp(1),n,plus,a,la,ws(na1),w,ls,snorm*tol)
...
c       print *,'only one degenerate c/s'
      ...
      if(ninf.gt.0)then
        alpha=amax
        if(plus)then
          call sp_mysaxpy(alpha,ws(na1),x,n)
        else
          call sp_mysaxpy(-alpha,ws(na1),x,n)
        endif
      else
...
c  calculate new x
        if(plus)then
          call sp_saxpyz(alpha,ws(na1),x,ws(nb1),n)
        else
          call sp_saxpyz(-alpha,ws(na1),x,ws(nb1),n)
        endif
        call sp_funct(n,ws(nb1),fp,ws,lws,cws,functions,gradients)
...
c         print 2,'df,z =',df,z
        ...
        f=fp
        call sp_grad(n,ws(nb1),g,ws,lws,cws,functions,gradients)
        ngr=ngr+1
c       print 4,'new g =',(g(i),i=1,n)
        call sp_newg
        gps=sp_scpr(0.D0,g,ws(na1),n)
        if(.not.plus)gps=-gps
...
c       print 4,'new rg =',(w(ls(j)),j=n-k+1,n)
        ...
        call sp_store_rg(k,ig1,ws(krg1),w,ls(n-k+1))
        gpg=0.D0
        gg=0.D0
        do j=n-k+1,n
          i=ls(j)
...
c           print 4,'c =',(ws(kc+i),i=1,ngv)
            call sp_formR(nv,k,ig,maxg,ws(ka1),ws(kb1),ws(kc1),ws(kd1),
     *        ws(ke1),ws(krg1),ws(kr1))
c           call sp_checkT(nv,maxg,ws(kr1),ws(ke1),ws(kd1))
            call sp_formT(nv,maxg,ws(kr1),v,ws(ke1))
c           print 4,'T matrix',(v(i),i=1,nv)
c             if(nv.gt.1)print 5,(ws(ke+i),i=1,nv-1)
            call sp_trid(v,ws(ke1),nv)
...
c  remove contribution to gradient
                if(i.gt.n)then
                  call sp_saipy(sign(1.D0,dble(ls(j))),a,la,i-n,g,n)
                else
                  g(i)=0.D0
                endif
              else
...
c  case: q is a free variable
          ...
          call sp_iexch(ls(qj),ls(n-k))
          ig=0
          if(n_inf.gt.0.and.ninf.eq.0)goto10
          goto15
        endif
        call sp_fbsub(n,n-k,n,a,la,q,w,w,ls,ws(lu1),lws(ll1),.false.)
...
c        if(p.eq.0)print *,'p.eq.0'
        if(p.eq.0)goto98
        call sp_pivot(p,q,n,nmi,a,la,e,ws(lu1),lws(ll1),ifail,npv)
        if(ifail.ge.1)then
...
c         print *,'r(q),w(q),q',r(q),w(q),q
        ...
        if(rp.gt.0.D0)then
c          if(phase.gt.0)print *,'phase =',phase
          call sp_iexch(ls(pj),ls(qj))
          call sp_iexch(ls(lp(1)),ls(qj))
          lp(1)=lp(1)-1
          if(ninf.gt.0)goto15
          goto9
        endif
...
c       if(abs(rp-r(p)).gt.1.D-1*gnorm)then
      ...
      snorm=e(p)
c  form At.s and denominators
      call sp_form_Ats(n1,lp(lev),n,plus,a,la,ws(na1),w,ls,snorm*tol)
   86 continue
      if(iprint.ge.3)then
        write(nout,1001)'residual vector and indices',
   ...
   90 continue
      if(qj.eq.0)then
        do j=n1,lp(lev)
          ...
          r(i)=0.D0
        enddo
        call sp_form_Ats(lp(lev)+1,lp(lev-1),n,plus,a,la,ws(na1),
     *    w,ls,snorm*tol)
        lev=lev-1
        f=alp(lev)
...
c1001 format(a/(i3,1x,e14.8,3(i4,1x,e14.8)))
     ...
     * /1111.D-19, 1.D-12, 0.D0, 1.D-8,  2,    3,   2,   500,   0,  0/
      ...
      subroutine sp_optest(jmin,jmax,r,e,ls,rp,pj)
      implicit double precision (a-h,r-z), integer (i-q)
      dimension r(*),e(*),ls(*)
      rp=0.D0
      do 1 j=jmin,jmax
        i=abs(ls(j))
c        if(e(i).eq.0.D0)print *,'e(i).eq.0.D0: i =',i
        ri=r(i)/e(i)
        if(ri.ge.rp)goto1
        rp=ri
        pj=j
    1 continue
      ...
      subroutine sp_form_Ats(jmin,jmax,n,plus,a,la,an,w,ls,tol)
      implicit double precision (a-h,r-z), integer (i-q)
...
c  form At.s and denominators
      if(plus)then
        do j=jmin,jmax
          i=abs(ls(j))
          if(i.gt.n)then
            wi=sp_aiscpr(n,a,la,i-n,an,0.D0)
          else
            wi=an(i)
          endif
          if(wi.ne.0.D0)then
      ...
      subroutine sp_zprod(k,n,a,la,an,r,w,ls,aa,ll)
      implicit double precision (a-h,r-z), integer (i-q)
      ...
      do j=n-k+1,n
        w(ls(j))=-r(ls(j))
      enddo
      call sp_tfbsub(n,a,la,0,w,an,aa,ll,ep,.false.)
      return
      end

      subroutine sp_signst(n,r,w,ls)
      implicit double precision (a-h,o-z)
      dimension r(*),w(*),ls(*)
c  transfer with sign change as necessary
      ...
      subroutine sp_stmap(n,nm,kmax,maxg)
...
c  remaining space for use by denseL.f or schurQR.f
      ...
      subroutine sp_warm_start(n,nm,a,la,x,bl,bu,b,ls,aa,ll,an,vstep)
      implicit double precision (a-h,o-z)
      dimension a(*),la(*),x(*),bl(*),bu(*),b(*),ls(*),
     *  aa(*),ll(*),an(*)
      DOUBLE PRECISION sp_daiscpr
      common/epsc/eps,tol,emin
      common/noutc/nout
      ...
      do j=1,n
        ...
        if(i.gt.n)then
          if(ls(j).ge.0)then
            b(i)=sp_daiscpr(n,a,la,i-n,x,-bl(i))
          else
            b(i)=sp_daiscpr(n,a,la,i-n,x,-bu(i))
          endif
        endif
      enddo
...
c     write(nout,1000)'r =',(b(abs(ls(j))),j=1,n)
      call sp_tfbsub(n,a,la,0,b,an,aa,ll,ep,.false.)
c     write(nout,1000)'d =',(an(i),i=1,n)
      call sp_linf(n,an,vstep,i)
      if(iprint.ge.1)
...
c1001 format(a/(i4,1x,e11.5,4(i4,1x,e11.5)))
      ...
      subroutine sp_residuals(n,jmin,jmax,a,la,x,bl,bu,r,ls,f,g,ninf)
      implicit double precision (a-h,r-z), integer (i-q)
      dimension a(*),la(*),x(*),bl(*),bu(*),r(*),ls(*),g(*)
      common/epsc/eps,tol,emin
      DOUBLE PRECISION sp_daiscpr,z
      f=0.D0
      ninf=0
      do j=jmin,jmax
        i=abs(ls(j))
        if(i.gt.n)then
          z=sp_daiscpr(n,a,la,i-n,x,0.D0)
        else
          z=dble(x(i))
        endif
        ri=z-dble(bl(i))
      ...
      subroutine sp_store_rg(k,ig,G,r,ls)
      implicit double precision (a-h,o-z)
      dimension G(k,*),r(*),ls(*)
      do j=1,k
        G(j,ig)=r(ls(j))
...
c  This routine is adapted from the EISPAK subroutine imtq11.
   ...
   15 continue
      ...
      subroutine sp_formR(nv,k,ig,maxg,a,b,c,d,e,G,R)
      implicit double precision (a-h,o-z)
   ...
   10 ii=ig-nv
      ...
      do i=1,nv
        ...
        do j=3,nvi1
          jj=jj+1
          if(jj.gt.maxg)jj=1
          d(j)=sp_scpr(0.D0,G(1,jj),G(1,ii),k)
        enddo
        ij=i
        do j=1,i-1
          call sp_mysaxpy(-R(ij),R(ij),d,nvi1)
          ij=ij+maxg-j
        enddo
        if(d(1).le.0.D0)then
...
c     enddo
    ...
    5 format((6E13.5))
      ...
      subroutine sp_formT(n,nmax,R,d,e)
      implicit double precision (a-h,o-z)
      dimension R(*),d(*),e(*)
...
c  insertion sort into ascending order
   ...
   10   continue
      ...
      subroutine sp_checkT(n,nmax,R,a,d)
      implicit double precision (a-h,o-z)
      dimension R(*),a(*),d(*)
      dimension T(10,10)
...
c     print 2,'a =',(a(i),i=1,n)
      do i=1,n
        call sp_mysaxpy(-1.D0,T(1,i+1),T(1,i),n)
        do j=1,n
          T(j,i)=T(j,i)*a(i)
        enddo
      enddo
...
c     enddo
    ...
    5 format((5E15.7))
      ...
      subroutine sp_check(n,nm,nmi,kmax,g,a,la,x,bl,bu,r,ls,an,f,
...
c         an(i)=an(i)-e
c       else
c         call sp_saipy(-e,a,la,i-n,an,n)
c       endif
c       goto1
c     endif
      j=nmi*(nmi+1)/2
...
c     if(e.gt.tol)stop
      if(e.gt.1.D-6)stop
      if(ninf.eq.0)then
        call sp_funct(n,x,ff,ws,lws,cws,functions,gradients)
        call sp_grad(n,x,an,ws,lws,cws,functions,gradients)
      else
        do i=1,n
          an(i)=0.D0
        enddo
        ...
        do j=n+1,nm
          ...
          if(r(i).lt.0.D0)then
            ff=ff-r(i)
            if(i.gt.n)then
              call sp_saipy(-sign(1.D0,dble(ls(j))),a,la,i-n,an,n)
            else
...
c         print *,'i,s,r(i)',i,s,r(i)
          call sp_saipy(-s*r(i),a,la,i-n,an,n)
          if(ls(j).gt.0)then
            s=sp_aiscpr(n,a,la,i-n,x,-bl(i))
          else
            s=-sp_aiscpr(n,a,la,i-n,x,-bu(i))
          endif
        endif
        if(abs(s).gt.e)then
          e=abs(s)
