...
path_new = get_absolute_file_path('mps_clp_test.sce');
path_old = pwd();
...
Log       = 1;
Debug     = 0;
mps_index = 1; // 1 LP - 2 MIP
// netlib index 5: fobj = nan apres 1000 iterations avec interior
mps_type  = 0;
UseLinpro = %F;
lp_method = 1;  // 6 interior - 7 pdco - other simplex
PrintSol  = %F;
PreSolve  = 1;
Perturb   = 1;
Scale     = 1;
...
param = add_param(param,'solver',lp_method);
...
param = add_param(param,'perturb',Perturb); // Boolean ?
...
param = add_param(param,'scaling',Scale); // Integer

param = add_param(param,'fact_freq',200); // Iterations
param = add_param(param,'presolve',PreSolve); // Boolean
//param = add_param(param,'red_grad',1); // Phase
...
param = add_param(param,'presolvetolerance',1e-7);
param = add_param(param,'maximumbarrieriterations',10000); // Defines the maximum iterations limits of clpinterior
...
[xmin,fmin,status,extra] = clp([],mps_file_mp('obj_coeff'),mps_file_mp('constr_mat'),mps_file_mp('lhs'),mps_file_mp('rhs'), ...
