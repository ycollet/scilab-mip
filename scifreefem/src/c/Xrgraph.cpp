...
myio& myio::operator << (const char *s) 
{ 
  cout<<s;
  return *this;
}
myio& myio::operator << (long int s) 
{  
  cout<<s;
  return *this;
}
myio& myio::operator << (double s)
{
  cout<<s;
  return *this;
}
...
//  void initdata(shl_t hndl, int loading)
//  {
//      void (*load)();
//      void (*load2)();
...
//      if (loading)
//      {
//  	shl_findsym(&hndl,"_GLOBAL_$I$monome__8Fonctionfi",TYPE_PROCEDURE, 
//  		    (void *) &load);
//  	shl_findsym(&hndl,"_GLOBAL_$I$Le__Fff",TYPE_PROCEDURE, 
//  		    (void *) &load2);
//      }
  ...
  if (INITGRAPH)
    {
      ...
      if(psfile) closePS();
      cerr << "Appel de closegraphique()" << endl;
    }
}
...
{ 
  ...
  if( !filename) 
    {
      do {
        sprintf(ffff,"xrgraph_%.3d.ps",count++);
	//	cout << " ps file =" << ffff << access(ffff,F_OK) << endl;
      } while ( access(ffff,F_OK) == 0  && count<1000);
