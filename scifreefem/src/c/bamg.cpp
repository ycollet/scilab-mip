...
const char whatbamg []= "@(#)INRIA, le 20 nov 1998,  Bamg v0.61  ";
  ...
  if (fgeom && fileout)
      ...
      if ( fileout) 
	{
	  ...
	  Triangles & Th( *(NoMeshReconstruction 
			    ...
			    :  new Triangles(nbvx,BTh)     // construct a new mesh 
			    ));
	  ...
	  if ( ( rbb && wbb)  ||( rBB && wBB))  // the code for interpolation 
		...
		*fbb  << dim << " " << nbsolbb << " " << Th.nbv <<" " << 2 << endl; 
	      ...
	      for(i=0;i<Th.nbv;i++)
		{
		  ...
		  Int4  ibb0 = nbsolbb*i0;
		  Int4  ibb1 = nbsolbb*i1;
		  Int4  ibb2 = nbsolbb*i2;
