...
#
## $Id: coin.m4 95 2006-07-12 00:37:48Z andreasw $
#
...
AC_DEFUN([AC_OBOE_PROJECTDIR_INIT],
...
if test x$oboe_projectdir = xyes; then
...
###########################################################################
#                        OBOE_HAS_USER_LIBRARY                            #
###########################################################################

# This macro sets up usage of a library with header files.  It defines
# the LBRYINCDIR variable, and it defines OBOE_HAS_LBRY preprocessor
...
AC_DEFUN([AC_OBOE_HAS_USER_LIBRARY],
[AC_REQUIRE([AC_OBOE_PROJECTDIR_INIT])
...
if test x"$$2INCDIR" != x || test x"$$2LIBDIR" != x; then
  m4_tolower(oboe_has_$2)=true
else
  m4_tolower(oboe_has_$2)=false
fi

if test $m4_tolower(oboe_has_$2) = true; then
# Check either both arguments or none are given
  if test x"$$2INCDIR" = x || test x"$$2LIBDIR" = x; then
    AC_MSG_ERROR([You need to specify both --with-m4_tolower($1)-incdir and --with-m4_tolower($1)-libdir and --with-m4_tolower($1)-lib if you want to use library $1])
  fi
  AC_MSG_RESULT(yes)
  ...
  m4_ifvaln([$4],[if test x"$m4_tolower($1)_libcheck" != xno; then
                    libval=$$2LIB
                    tmp_save_LIBS="$LIBS"
                    LIBS="-L$$2LIBDIR $$2LIBDIR/$$2LIB $ADDLIBS"
		    AC_MSG_CHECKING([$LIBS whether symbol $4 is available with $2])
...
#                    AC_TRY_LINK([extern "C" {void $4();}],[$4()],
  ...
  ADDLIBS="-L$$2LIBDIR -l$$2LIB $ADDLIBS"
  AC_DEFINE(OBOE_HAS_$2,[1],[Define to 1 if the $1 package is used])
else
  AC_MSG_RESULT(no)
fi
...
AC_SUBST($2LIBDIR)
AC_SUBST($2LIB)
AM_CONDITIONAL(OBOE_HAS_$2,
               test $m4_tolower(OBOE_has_$2) = true)
]) #AC_OBOE_HAS_USER_LIBRARY
